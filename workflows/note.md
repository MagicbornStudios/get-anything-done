...
- **Global scope**: `~/.gad/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/gad:note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
2. Ensure the notes directory exists (`.planning/notes/` or `~/.gad/notes/`)
...
2. Glob `~/.gad/notes/*.md` (if directory exists) — global notes
...
{count} active note(s). Use `/gad:note promote <N>` to convert to a todo.
```
...
4. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a GAD project. Run `/gad:new-project` to initialize one."
...
<edge_cases>
1. **"list" as note text**: `/gad:note list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `.planning/`**: Falls back to global `~/.gad/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `.planning/`, suggests `/gad:new-project`
