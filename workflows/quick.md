...
```bash
INIT=$(node "$HOME/.claude/bin/gad-tools.cjs" init quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(node "$HOME/.claude/bin/gad-tools.cjs" agent-skills gad-planner 2>/dev/null)
AGENT_SKILLS_EXECUTOR=$(node "$HOME/.claude/bin/gad-tools.cjs" agent-skills gad-executor 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/bin/gad-tools.cjs" agent-skills gad-checker 2>/dev/null)
AGENT_SKILLS_VERIFIER=$(node "$HOME/.claude/bin/gad-tools.cjs" agent-skills gad-verifier 2>/dev/null)
```
...
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
