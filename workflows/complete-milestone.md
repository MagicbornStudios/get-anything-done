...
2. **Run audit first** — `/gad:audit-milestone` to assess gap severity
...
```bash
cat gad-config.toml 2>/dev/null || cat .planning/config.json 2>/dev/null || true
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  gad-tools summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
```bash
ARCHIVE=$(gad-tools milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**If doesn't exist:** Create from template at `templates/retrospective.md`.
...
**Commit:**
```bash
gad-tools commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(gad-tools init execute-phase "1")
...
Detect base branch:
```bash
BASE_BRANCH=$(gad-tools config-get git.base_branch 2>/dev/null || echo "")
...
```bash
gad-tools commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/gad:new-milestone)
