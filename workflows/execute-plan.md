...
```bash
gad-tools phases list --type summaries --raw
...
```bash
gad-tools commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
gad-tools state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
gad-tools roadmap update-plan-progress "${PHASE}"
```
...
```bash
gad-tools commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/gad:execute-phase {phase}` + `/gad:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/gad:plan-phase {Z+1}` + `/gad:verify-work {Z}` + `/gad:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/gad:complete-milestone` + `/gad:verify-work` + `/gad:add-phase` |
