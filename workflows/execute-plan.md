...
@~/.claude/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/bin/gad-tools.cjs" init execute-phase "${PHASE}")
...
# config settings can be fetched via gad-tools config-get if needed
```
...
2. Per segment:
   - Subagent route: spawn gad-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node ~/.claude/bin/gad-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See ~/.claude/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/bin/gad-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/templates/summary.md`.
...
Update STATE.md using gad-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/bin/gad-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/bin/gad-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/bin/gad-tools.cjs" state record-metric \
...
node "$HOME/.claude/bin/gad-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/bin/gad-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using gad-tools:
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
