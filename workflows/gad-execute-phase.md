...
<legacy_methodology>
The older long-form `execute-phase` skill carried the discipline contract that still governs execution:

- One task is one unit of work: implement -> verify -> commit -> update planning docs.
- Decisions discovered during implementation must be written down before the phase closes.
- Verification is not optional; if verify is broken, surface it rather than silently pushing forward.
- Phase close requires an explicit definition-of-done check and a verification artifact.

Per-task expectations remain:
- mark task active
- implement only the task scope
- run the task's verify command
- commit atomically
- update TASK-REGISTRY, STATE, and any needed DECISIONS
</legacy_methodology>
...
- **Claude Code:** Uses `Task(subagent_type="gad-executor", ...)` — blocks until complete, returns result
...
- gad-integration-checker — Checks cross-phase integration
- gad-nyquist-auditor — Validates verification coverage
- gad-ui-researcher — Researches UI/UX approaches
- gad-ui-checker — Reviews UI implementation quality
- gad-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
```bash
INIT=$(gad-tools init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(gad-tools agent-skills gad-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(gad-tools config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
```bash
CONTEXT_WINDOW=$(gad-tools config-get context_window 2>/dev/null || echo "200000")
```
...
Check if the current runtime is Copilot by testing for the `@gad-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  gad-tools config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
```bash
gad-tools state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(gad-tools phase-plan-index "${PHASE_NUMBER}")
```
   ...
   **Worktree mode** (`USE_WORKTREES` is not `false`):
   ...
   Task(
     ...
     prompt="
       ...
       - gad-config.toml (canonical config, if present)
       - .planning/config.json (compatibility mirror, if present)
...
5. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a GAD or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 4 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    gad-tools verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
**Security gate check:**
```bash
SECURITY_CFG=$(gad-tools config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
...
```bash
POST_PLAN_INDEX=$(gad-tools phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
PARENT_INFO=$(gad-tools find-phase "${PARENT_PHASE}" --raw)
...
```bash
gad-tools commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
SCHEMA_DRIFT=$(gad-tools verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
Check for override:
```bash
SKIP_SCHEMA=$(echo "${GAD_SKIP_SCHEMA_CHECK:-false}")
```
...
Display:
```
⚠ Schema drift detected but GAD_SKIP_SCHEMA_CHECK=true — bypassing gate.
...
2. Skip schema check (GAD_SKIP_SCHEMA_CHECK=true) — bypass this gate
...
```bash
VERIFIER_SKILLS=$(gad-tools agent-skills gad-verifier 2>/dev/null)
```
...
| `gaps_found` | Present gap summary, offer `/gad:plan-phase {phase} --gaps ${GAD_WS}` |
...
Commit the file:
```bash
gad-tools commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/gad:progress` and `/gad:audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/gad:verify-work` on it.
...
`/gad:plan-phase {X} --gaps ${GAD_WS}`

<sub>`/clear` first → fresh context window</sub>

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/gad:verify-work {X} ${GAD_WS}` — manual testing first
```

Gap closure cycle: `/gad:plan-phase {X} --gaps ${GAD_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/gad:execute-phase {X} --gaps-only ${GAD_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(gad-tools phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/gad:progress` and `/gad:audit-uat`.
```

```bash
gad-tools commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
gad-tools commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/gad:plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
**IMPORTANT: There is NO `/gad:transition` command. Never suggest it. The transition workflow is internal only.**

```
## ✓ Phase {X}: {Name} Complete

/gad:progress ${GAD_WS} — see updated roadmap
/gad:discuss-phase {next} ${GAD_WS} — discuss next phase before planning
/gad:plan-phase {next} ${GAD_WS} — plan next phase
/gad:execute-phase {next} ${GAD_WS} — execute next phase
```
...
<resumption>
Re-run `/gad:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
