...
```bash
INIT=$(gad-tools init milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/gad:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/gad:new-milestone` first."
...
```bash
ROADMAP=$(gad-tools roadmap analyze)
```
...
```bash
DETAIL=$(gad-tools roadmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GAD ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(gad-tools init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(gad-tools config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
gad-tools commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="gad:discuss-phase", args="${PHASE_NUM}")
```
...
```bash
PHASE_SECTION=$(gad-tools roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```
Skill(skill="gad:ui-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"gad:plan-phase\", args=\"${PHASE_NUM}\")"
)
```
...
```
Skill(skill="gad:plan-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Skill(skill=\"gad:execute-phase\", args=\"${PHASE_NUM} --no-transition\")"
)
```
...
```
Skill(skill="gad:execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="gad:plan-phase", args="${PHASE_NUM} --gaps")
```
...
```bash
UI_REVIEW_CFG=$(gad-tools config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```
Skill(skill="gad:ui-review", args="${PHASE_NUM}")
```
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `gad:discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `discuss-phase` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
gad-tools commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GAD ► AUTONOMOUS ▸ PHASE ${ONLY_PHASE} COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Phase ${ONLY_PHASE}: ${PHASE_NAME} — Done
 Mode: Single phase (--only)

 Lifecycle skipped — run /gad:autonomous without --only
...
```
Skill(skill="gad:audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /gad:audit-milestone to review, then /gad:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /gad:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="gad:complete-milestone", args="${milestone_version}")
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /gad:autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}
```
...
- [ ] Audit invoked via Skill(skill="gad:audit-milestone")
...
- [ ] `--interactive` runs discuss inline via gad:discuss-phase (asks questions, waits for user)
