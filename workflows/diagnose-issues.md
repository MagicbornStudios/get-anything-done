...
```bash
AGENT_SKILLS_DEBUGGER=$(gad-tools agent-skills gad-debugger 2>/dev/null)
```
...
```bash
gad-tools commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
