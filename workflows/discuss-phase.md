...
Use /gad:progress ${GAD_WS} to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /gad:plan-phase {X} ${GAD_WS} to replan
...
**Research-before-questions mode:** Check if `workflow.research_before_questions` is enabled in config (from init context or `gad-config.toml`). When enabled, before presenting questions for each area:
...
**Also available:**
- `/gad:discuss-phase ${PHASE} --chain ${GAD_WS}` — re-run with auto plan+execute after
- `/gad:plan-phase ${PHASE} --skip-research ${GAD_WS}` — plan without research
- `/gad:ui-phase ${PHASE} ${GAD_WS}` — generate UI design contract before planning (if phase has frontend work)
