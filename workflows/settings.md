<purpose>
Interactive configuration of GAD workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates `gad-config.toml` and regenerates `.planning/config.json` for compatibility. Optionally saves settings as global defaults (~/.gad/defaults.json) for future projects.
</purpose>
...
```bash
gad-tools config-ensure-section
INIT=$(gad-tools state load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Creates `gad-config.toml` with defaults if missing and regenerates `.planning/config.json` for compatibility.
</step>
...
- `workflow.ui_safety_gate` — prompt to run /gad:ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /gad:ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /gad:ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (gad/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (gad/{version}-{name})" }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /gad:autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  }
])
```
...
Write updated config to `gad-config.toml` and regenerate `.planning/config.json`.
</step>
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.gad/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.gad/defaults.json`:

```bash
mkdir -p ~/.gad
```

Write `~/.gad/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /gad:plan-phase and /gad:execute-phase runs.

Quick commands:
- /gad:set-profile <profile> — switch model profile
- /gad:plan-phase --research — force research
- /gad:plan-phase --skip-research — skip research
- /gad:plan-phase --skip-verify — skip plan check
```
</step>
...
- [ ] User offered to save as global defaults (~/.gad/defaults.json)
