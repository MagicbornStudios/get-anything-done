...
```bash
INIT=$(node "$HOME/.claude/bin/gad-tools.cjs" docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.claude/bin/gad-tools.cjs" agent-skills gad-doc-writer 2>/dev/null)
```
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs: generate project documentation" \
