...
```bash
INIT=$(node "$HOME/.claude/bin/gad-tools.cjs" init progress)
...
- `STATE=$(node "$HOME/.claude/bin/gad-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "$HOME/.claude/bin/gad-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node "$HOME/.claude/bin/gad-tools.cjs" progress bar --raw)
```
...
```bash
DEBT=$(node "$HOME/.claude/bin/gad-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/bin/gad-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$(node "$HOME/.claude/bin/gad-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
