...
```bash
INIT=$(node "$HOME/.claude/bin/gad-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.claude/bin/gad-tools.cjs" agent-skills gad-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(node "$HOME/.claude/bin/gad-tools.cjs" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
```bash
CONTEXT_WINDOW=$(node "$HOME/.claude/bin/gad-tools.cjs" config-get context_window 2>/dev/null || echo "200000")
```
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "$HOME/.claude/bin/gad-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/workflows/execute-plan.md` **inline**
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$HOME/.claude/bin/gad-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
   ...
   **Worktree mode** (`USE_WORKTREES` is not `false`):
   ...
   Task(
     ...
     prompt="
       ...
       @~/.claude/references/checkpoints.md
       @~/.claude/references/tdd.md
       </execution_context>
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$HOME/.claude/bin/gad-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "$HOME/.claude/bin/gad-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "$HOME/.claude/bin/gad-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**Security gate check:**
```bash
SECURITY_CFG=$(node "$HOME/.claude/bin/gad-tools.cjs" config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/bin/gad-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
PARENT_INFO=$(node "$HOME/.claude/bin/gad-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
SCHEMA_DRIFT=$(node "$HOME/.claude/bin/gad-tools.cjs" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.claude/bin/gad-tools.cjs" agent-skills gad-verifier 2>/dev/null)
```
...
Commit the file:
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
COMPLETION=$(node "$HOME/.claude/bin/gad-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `~/.claude/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
