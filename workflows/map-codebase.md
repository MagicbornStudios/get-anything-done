...
```bash
INIT=$(node "$HOME/.claude/bin/gad-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$HOME/.claude/bin/gad-tools.cjs" agent-skills gad-codebase-mapper 2>/dev/null)
```
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
