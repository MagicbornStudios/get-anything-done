...
AGENT_SKILLS_AUDITOR=$(gad-tools agent-skills gad-nyquist-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(gad-tools resolve-model gad-nyquist-auditor --raw)
NYQUIST_CFG=$(gad-tools config-get workflow.nyquist_validation --raw)
```
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /gad:execute-phase {N} ${GAD_WS} first."
...
```
Task(
  prompt="Read @agents/gad-nyquist-auditor.md for instructions.\n\n" +
...
gad-tools commit "docs(phase-${PHASE}): add/update validation strategy"
```
...
▶ Next: /gad:audit-milestone ${GAD_WS}
```
...
▶ Retry: /gad:validate-phase {N} ${GAD_WS}
```
