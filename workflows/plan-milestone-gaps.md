<purpose>
Create all phases necessary to close gaps identified by `/gad:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/gad:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/gad:audit-milestone` first.
```
...
HIGHEST=$(gad-tools phases list --pick directories[-1])
```
...
```bash
gad-tools commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/gad:execute-phase {N}` — if plans already exist
...
`/gad:audit-milestone` — re-audit to verify gaps closed
`/gad:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/gad:plan-phase` next
</success_criteria>
