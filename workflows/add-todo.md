...
```bash
INIT=$(node "$HOME/.claude/bin/gad-tools.cjs" init todos)
...
```bash
slug=$(node "$HOME/.claude/bin/gad-tools.cjs" generate-slug "$title" --raw)
```
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
