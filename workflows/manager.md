...
All {phase_count} phases done. Ready for final steps:
  → /gad:verify-work — run acceptance testing
  → /gad:complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="gad:verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="gad:complete-milestone")` then exit.
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in GAD config (default: 60 seconds, set to 0 to disable).
...
Task(
  ...
  prompt="You are running the GAD plan-phase workflow for phase {N} of the project.
...
Skill(skill=\"gad:plan-phase\", args=\"{N} --auto\")
...
Task(
  ...
  run_in_background=true,
  prompt="You are running the GAD execute-phase workflow for phase {N} of the project.
...
Skill(skill=\"gad:execute-phase\", args=\"{N}\")
...
- Display the error clearly, then offer to fix it:
  ...
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill — use `Skill(skill="gad:plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="gad:execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill — use `Skill(skill="gad:plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="gad:execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /gad:manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/gad:manager` or `/gad:progress` invocation.
