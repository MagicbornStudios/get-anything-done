...
- Prefer useful reference docs over arbitrary brevity.
- Always include concrete file paths in examples and claims.
- The codebase map is only worth generating if it helps planning and execution later.
...
If a mapper output cannot tell a later agent where to look in the codebase, it is too vague.
</legacy_methodology>
...
```bash
INIT=$(gad-tools init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(gad-tools agent-skills gad-codebase-mapper 2>/dev/null)
```
...
> The timeout is configurable via `workflow.subagent_timeout` in `gad-config.toml` (milliseconds). Default: 300000 (5 minutes). Increase for large codebases or slower models.
...
```bash
gad-tools commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/gad:map-codebase`
...
- User offered clear next steps in GAD style
</success_criteria>
