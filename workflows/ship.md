...
```bash
gad-tools state update "Last Activity" "$(date +%Y-%m-%d)"
gad-tools state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
gad-tools commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
...
- /gad:complete-milestone (if last phase in milestone)
- /gad:progress (to see what's next)
...
- /gad:complete-milestone — if all phases in milestone are done
- /gad:progress — see overall project state
- /gad:execute-phase {next} — continue to next phase
</offer_next>
