...
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/bin/gad-tools.cjs" agent-skills gad-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.claude/bin/gad-tools.cjs" resolve-model gad-integration-checker --raw)
```
...
node "$HOME/.claude/bin/gad-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/bin/gad-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/bin/gad-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/bin/gad-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
