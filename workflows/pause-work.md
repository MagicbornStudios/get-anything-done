<purpose>
Create structured `.planning/HANDOFF.json` and `.continue-here.md` handoff files to preserve complete work state across sessions. The JSON provides machine-readable state for `/gad:resume-work`; the markdown provides human-readable context.
</purpose>
...
```bash
timestamp=$(gad-tools current-timestamp full --raw)
```
...
<step name="commit">
```bash
gad-tools commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md .planning/HANDOFF.json
```
</step>
