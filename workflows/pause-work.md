...
```bash
timestamp=$(node "$HOME/.claude/bin/gad-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md .planning/HANDOFF.json
```
</step>
