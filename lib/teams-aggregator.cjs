...
function gatherWorkers(planningDir) {
  ...
  return ids.map((id) => {
    ...
    return {
      ...
      parking_enabled: status.parking_enabled ?? null,
...
function gatherCooldowns(planningDir) {
  const cooldownPath = path.join(planningDir, 'team', 'runtime-cooldown.json');
  const raw = readJson(cooldownPath) || {};
  const now = Date.now();
  return Object.entries(raw).map(([runtime, info]) => {
