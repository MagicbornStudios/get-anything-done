...
function generateScore(projectDir, version) {
  ...
  // Load gad.json for build_requirement gate
  let gadJson = {};
  try {
    const cfgPath = path.join(projectDir, 'gad.json');
    if (fs.existsSync(cfgPath)) gadJson = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
  } catch {}
  ...
  const buildRequirement = gadJson.build_requirement || trace.build_requirement || null;
