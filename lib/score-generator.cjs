...
function generateScore(projectDir, version) {
  ...
  // The project-level `gad.json` that used to carry `build_requirement`
  // was renamed to species-level `species.json` in task 42.4-14 (decision
  // gad-184). This reader only needs the `build_requirement` gate field,
  // which lives on the resolved species now (`buildRequirement`, camelCase
  // per task 42.4-19). Fall back to `trace.build_requirement` if the
  // species metadata cannot be resolved.
  let speciesBuildRequirement = null;
  try {
    const { loadAllResolvedSpecies } = require('./eval-loader.cjs');
    const resolved = loadAllResolvedSpecies(projectDir);
    for (const sp of resolved) {
      if (sp && sp.buildRequirement) { speciesBuildRequirement = sp.buildRequirement; break; }
    }
  } catch {}
  ...
  const buildRequirement = speciesBuildRequirement || trace.build_requirement || null;
  let buildGatePassed = true;
  if (buildRequirement) {
    // Check if a preserved build exists for this version
    const repoRoot = path.resolve(projectDir, '..', '..', '..', '..');
    const preservedBuild = path.join(repoRoot, 'apps', 'portfolio', 'public', 'evals', projectName, version);
    const runBuild = path.join(runDir, 'run', 'dist');
    const hasBuild = fs.existsSync(preservedBuild) || fs.existsSync(runBuild);
    const hasHumanReview = trace.human_review?.score != null || trace.scores?.human_review != null;
    buildGatePassed = hasBuild;

    lines.push('## Build requirement gate');
    lines.push('');
    lines.push('| Check | Status |');
    lines.push('|-------|--------|');
    lines.push(`| Requirement | ${buildRequirement} |`);
    lines.push(`| Build produced | ${hasBuild ? 'PASS' : 'FAIL'} |`);
    lines.push(`| Human reviewed | ${hasHumanReview ? 'PASS' : 'pending'} |`);
    lines.push(`| **Gate** | **${buildGatePassed ? 'PASSED' : 'FAILED'}** |`);
    lines.push('');
    if (!buildGatePassed) {
      lines.push('> **This eval is NOT complete.** The build requirement was not met — no build artifact found.');
      lines.push('> Scores below are provisional and should not be used for comparison.');
      lines.push('');
    }
  }
