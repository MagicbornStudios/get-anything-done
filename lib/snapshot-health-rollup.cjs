...
const SINGLETONS = [
  { id: 'overnight', pidfile: 'overnight.pid' },
  { id: 'datasets-curator', pidfile: 'datasets-curator.pid' },
  { id: 'sessions-watcher', pidfile: 'sessions-watcher.pid' },
  { id: 'accounts-poller', pidfile: 'accounts-poller.pid' },
];
...
function buildHealthSection(baseDir) {
  ...
  const deadDaemons = daemons.filter((d) => !d.alive && d.exists);
  if (deadDaemons.length > 0) {
    hints.push(`! Daemons crashed (stale pidfile): ${deadDaemons.map((d) => d.id).join(', ')}.  Run: gad system start`);
  }
  const offDaemons = daemons.filter((d) => !d.alive && !d.exists);
  if (offDaemons.length === SINGLETONS.length) {
    hints.push(`! No daemons running.       Run: gad system start --projectid <id>`);
  }
