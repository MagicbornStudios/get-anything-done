 ...
 *   - createHandoff({ baseDir, projectid, phase, taskId, priority, estimatedContext, body, createdBy, runtimePreference, runtimeFallbacks, runtimeRequired, fsImpl })
 *
 * Stable error codes:
...
function unclaimHandoff({ baseDir, id, reason = '', by = '', fsImpl } = {}) {
  ...
  const text = fsi.readFileSync(location.filePath);
  const { frontmatter, body } = parseFrontmatter(text);
  const unclaimHistory = Array.isArray(frontmatter.unclaim_history) ? frontmatter.unclaim_history.slice() : [];
  unclaimHistory.push({
  ...
  frontmatter.claimed_by = '';
  frontmatter.claimed_at = '';
  frontmatter.unclaim_history = unclaimHistory;
