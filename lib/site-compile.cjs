...
/**
 * @param {object} opts
 * @param {string} opts.rootDir - Absolute path to static site root (index.html + assets).
 * @param {number} [opts.port] - TCP port (default 3456).
 * @param {string} [opts.host] - Bind address (default 127.0.0.1).
 * @param {string} [opts.logPrefix] - Prefix for console lines (default `[gad site]`).
 * @param {(server: object) => void} [opts.onListening] - Fires once when listen succeeds.
 */
function serveStatic({ rootDir, port, host, logPrefix = '[gad site]', onListening } = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    // Resolve in priority order. The tricky case: Next.js app router often
    // produces BOTH `foo.html` (the /foo route) AND `foo/` (a directory
    // holding nested routes like /foo/[id]). We need to serve foo.html when
    // someone asks for /foo, not treat foo/ as a directory fallback.
    //
    // Priority:
    //   1. Exact file with the extension the URL already has
    //      (/foo.html → foo.html, /_next/static/x.js → x.js)
    //   2. Sibling .html file — /foo → foo.html  (app router route)
    //   3. Directory index — /foo → foo/index.html  (non-Next sites)
    //   4. 404
    let filePath = null;
    ...
    if (!filePath) {
      // Try 404.html fallback (Next.js _not-found renamed during extract)
  ...
  server.listen(port_, host_, () => {
    console.log(`${logPrefix} serving ${rootDir}`);
    console.log(`${logPrefix} http://${host_}:${port_}/`);
