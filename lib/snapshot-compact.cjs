 ...
 *
 * @param {string} xml
 * @param {string} [nextActionOverride] - optional derived next action
 * @returns {string}
 */
function compactStateXml(xml, nextActionOverride) {
  ...
  const nextActionXml = getText('next-action');
  const nextAction = nextActionOverride || nextActionXml;
  ...
  if (refs.length > 0) {
    lines.push('refs:');
    if (refs.length >= REFS_TREE_THRESHOLD) {
      lines.push(compactRefsTree(refs));
    } else {
...
const REFS_TREE_THRESHOLD = 5;

function splitRef(ref) {
  const parenIdx = ref.indexOf(' (');
  const annotation = parenIdx >= 0 ? ref.slice(parenIdx) : '';
  const pathPart = parenIdx >= 0 ? ref.slice(0, parenIdx) : ref;
  const isDir = pathPart.endsWith('/');
  const cleaned = isDir ? pathPart.slice(0, -1) : pathPart;
  return { segments: cleaned.split('/').filter(Boolean), annotation, isDir };
}

function buildRefsTree(refs) {
  const root = { dirs: new Map(), leaves: [] };
  for (const ref of refs) {
    const { segments, annotation, isDir } = splitRef(ref);
    ...
    node.leaves.push({ name: isDir ? `${fname}/` : fname, annotation });
  }
  return root;
}

function collapseDirChain(node) {
  let key = '';
  let n = node;
  while (n.dirs.size === 1 && n.leaves.length === 0) {
    const [childName, grandchild] = n.dirs.entries().next().value;
    key = key ? `${key}/${childName}` : childName;
    n = grandchild;
  }
  return { key, node: n };
}

function renderRefsNode(node, indent) {
  const lines = [];
  const leaves = [...node.leaves].sort((a, b) => a.name.localeCompare(b.name));
  for (const leaf of leaves) lines.push(`${indent}${leaf.name}${leaf.annotation}`);
  const dirs = [...node.dirs.entries()].sort(([a], [b]) => a.localeCompare(b));
  for (const [name, child] of dirs) {
    const collapsed = collapseDirChain(child);
    const fullName = collapsed.key ? `${name}/${collapsed.key}` : name;
    // Fold when the collapsed subtree holds exactly one leaf and no further dirs.
    if (collapsed.node.dirs.size === 0 && collapsed.node.leaves.length === 1) {
      const lone = collapsed.node.leaves[0];
      lines.push(`${indent}${fullName}/${lone.name}${lone.annotation}`);
    } else {
      lines.push(`${indent}${fullName}/`);
      lines.push(...renderRefsNode(collapsed.node, `${indent}  `));
    }
  }
  return lines;
}
...
/**
 * Render a refs list as an indented tree with shared-prefix compression.
 *
 * Collapses single-child directory chains (e.g. `sdk/src/file.ts` stays one
 * line rather than `sdk/` → `src/` → `file.ts`). Keeps parenthetical
 * annotations pinned to their leaf.
 *
 * Below REFS_TREE_THRESHOLD entries the flat form is cheaper; above it the
 * tree form cuts repeated path prefixes. Decision: gad-241.
 *
 * @param {string[]} refs
 * @returns {string}
 */
function compactRefsTree(refs) {
  if (!refs || refs.length === 0) return '';
  if (refs.length < REFS_TREE_THRESHOLD) {
    return refs.map(r => `- ${r}`).join('\n');
  }
  const tree = buildRefsTree(refs);
  return renderRefsNode(tree, '  ').join('\n');
}
  ...
  return content
    ...
    .map(line => {
      ...
      if (taskMatch) {
        ...
        const meta = parts.length > 0 ? ` [${parts.join(', ')}]` : '';
        return `  ${id}${meta} ${decodeEntities(body.trim())}`;
      }
      return line;
    })
    .join('\n');
