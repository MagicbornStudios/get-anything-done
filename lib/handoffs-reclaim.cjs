 ...
 *   reclaimDeadWorkerClaims({ baseDir, workerIds, dryRun }) → { reclaimed, skipped }
 *
 * Algorithm:
...
const DEFAULT_STALE_AFTER_MS = getSetting('handoffs.reclaim.stale_after_ms', 90 * 1000);
const HEARTBEAT_STALE_MS = getSetting('handoffs.heartbeat.stale_after_ms', 30 * 1000);
...
function classifyClaimer({ baseDir, claimedBy, handoffId, now }) {
  ...
  if (workerId) {
    ...
    if (Number.isInteger(status.pid) && !pidAlive(status.pid)) {
      return { dead: true, evidence: `worker ${workerId} pid=${status.pid} not alive` };
    }
...
function reclaimDeadWorkerClaims({
  baseDir,
  workerIds = [],
  dryRun = false,
} = {}) {
  if (!baseDir) throw new Error('reclaimDeadWorkerClaims: baseDir is required');
  ...
  for (const entry of entries) {
    ...
    const { frontmatter } = parseFrontmatter(text);
    const claimer = frontmatter.claimed_by || '';
    const workerId = extractWorkerId(claimer);

    if (!workerId || (workerSet.size > 0 && !workerSet.has(workerId))) {
      continue;
    }
