 ...
 *   2. GAD team state — .planning/team/state.json for multi-agent activity
...
function readOmxState(baseDir) {
  ...
  // OMX/claw-code removed entirely 2026-04-22 — no legacy fallback.
  const statePath = path.join(baseDir, '.planning', 'team', 'state.json');
