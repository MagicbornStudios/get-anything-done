...
/**
 * Infer per-(runtime, session) liveness from GAD trace-events.jsonl.
 *
 * Rationale (operator decision 2026-04-19): explicit `gad agents heartbeat`
 * is useless overhead when every agent already spams the GAD CLI (each
 * call emits a trace event via bin/gad-trace-hook.cjs). Last-seen-in-trace
 * IS the liveness signal.
 *
 * Reads root `.planning/.trace-events.jsonl` (rotated at 1000 lines).
 * Groups by (runtime.id, runtime.session_id), keeps latest ts per group.
 * Returns array sorted most-recent first:
 *   [{ runtime, sessionId, lastSeenAt, ageMs, stale, lastTool, eventCount }]
 *
 * Stale threshold default 10 min — adjust via opts.staleMs.
 */
function inferLivenessFromTrace(baseDir, { staleMs = 10 * 60 * 1000, tracePath } = {}) {
  const filePath = tracePath || path.join(baseDir, '.planning', '.trace-events.jsonl');
