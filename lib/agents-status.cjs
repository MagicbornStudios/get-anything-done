...
/**
 * Heartbeat files at <baseDir>/.gad/heartbeats/<runtime>-<session>.json.
 * Each runtime (or its wrapper) writes a heartbeat with { last_seen_at,
 * pid, runtime, session_id, cwd } every N seconds. Stale heartbeats
 * (>staleMs old) are flagged.
 *
 * Returns array of { runtime, sessionId, pid, cwd, lastSeenAt, ageMs,
 * stale, heartbeatPath }.
 */
function listHeartbeats(baseDir, { staleMs = 90 * 1000 } = {}) {
  const dir = path.join(baseDir, '.gad', 'heartbeats');
  ...
  for (const file of files) {
    ...
    const data = readJsonSafe(full);
    if (!data) continue;
    const age = ageMs(data.last_seen_at);
    out.push({
      ...
      heartbeatPath: path.relative(baseDir, full),
    });
  }
...
/**
 * Write a heartbeat for the current process.
 * Returns { path, data }.
 */
function writeHeartbeat(baseDir, { runtime, sessionId, cwd }) {
  const dir = path.join(baseDir, '.gad', 'heartbeats');
  fs.mkdirSync(dir, { recursive: true });
  const safeSession = String(sessionId || `pid-${process.pid}`).replace(/[^a-zA-Z0-9._-]+/g, '_');
  const filename = `${runtime || 'unknown'}-${safeSession}.json`;
