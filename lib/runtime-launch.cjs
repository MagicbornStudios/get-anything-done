...
// Default: silent (no visible PowerShell window, no -NoExit).
// Set GAD_RUNTIME_LAUNCH_VISIBLE=1 (or "true"/"yes") to opt into a
// visible debug shell — useful for watching runtime output interactively.
// The old GAD_RUNTIME_LAUNCH_HIDE env var is no longer needed and is ignored.
function launchRuntimeInNewShellWindows({ command, args = [], cwd, runtimeId = 'runtime' }) {
  ...
  const visible = /^(1|true|yes)$/i.test(String(process.env.GAD_RUNTIME_LAUNCH_VISIBLE || ''));
  const psArgs = visible
    ? ['-NoLogo', '-NoExit', '-ExecutionPolicy', 'Bypass', '-File', scriptPath]
    : ['-NoLogo', '-ExecutionPolicy', 'Bypass', '-File', scriptPath];
