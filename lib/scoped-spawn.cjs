 ...
 * 60-02's `acquire()` throws `KEY_NOT_FOUND` with message "no envelope for
 * project ..." when the `.gad/secrets/<id>.enc` file is missing. That is a
 * different semantic than "the requested key name is not in the bag" — we
 * treat the former as PROJECT_NOT_FOUND so callers can decide to proceed
 * with no project keys (default) or abort (requireBag=true).
 *
 * @param {Error} err
 * @returns {boolean}
 */
function isMissingEnvelope(err) {
  if (!err) return false;
  // Recognize both the raw store-error code and the common message pattern.
  if (err.code === 'PROJECT_NOT_FOUND') return true;
  if (err.code === 'KEY_NOT_FOUND' && typeof err.message === 'string'
      && /no envelope for project/i.test(err.message)) {
    return true;
  }
  return false;
}
