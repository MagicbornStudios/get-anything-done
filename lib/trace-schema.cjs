...
function makeAgentEnvelope(agent) {
  return {
    agent_id: agent?.agent_id ?? null,
    agent_role: agent?.agent_role ?? null,
    parent_agent_id: agent?.parent_agent_id ?? null,
    root_agent_id: agent?.root_agent_id ?? null,
    depth: Number.isFinite(agent?.depth) ? Number(agent.depth) : null,
    model_profile: agent?.model_profile ?? null,
    resolved_model: agent?.resolved_model ?? null,
  };
}
 ...
 * }
 */
function makeToolUseEvent({ seq, runtime, agent, tool, inputs, outputs, outputsTruncated, durationMs, success, triggerSkill }) {
  return {
    ts: new Date().toISOString(),
    seq,
    type: EVENT_TYPES.TOOL_USE,
    runtime: makeRuntimeEnvelope(runtime),
    agent: makeAgentEnvelope(agent),
    tool,
    inputs: inputs ?? null,
 ...
 * }
 */
function makeSkillInvocationEvent({ seq, runtime, agent, skillId, parent, triggerContext, triggerSnippet }) {
  return {
    ts: new Date().toISOString(),
    seq,
 ...
 * }
 */
function makeSubagentSpawnEvent({ seq, runtime, agent, agentId, inputs, outputs, durationMs, success }) {
  return {
    ts: new Date().toISOString(),
    seq,
 ...
 * }
 */
function makeFileMutationEvent({ seq, runtime, agent, filePath, op, sizeDelta }) {
  return {
    ts: new Date().toISOString(),
    seq,
