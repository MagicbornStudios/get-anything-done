 ...
 * }
 */
function makeToolUseEvent({ seq, tool, inputs, outputs, outputsTruncated, durationMs, success, triggerSkill }) {
  return {
 ...
 * }
 */
function makeSkillInvocationEvent({ seq, skillId, parent, triggerContext, triggerSnippet }) {
  return {
    ts: new Date().toISOString(),
    seq,
 ...
 * }
 */
function makeSubagentSpawnEvent({ seq, agentId, inputs, outputs, durationMs, success }) {
  return {
    ts: new Date().toISOString(),
    seq,
 ...
 * }
 */
function makeFileMutationEvent({ seq, filePath, op, sizeDelta }) {
  return {
    ts: new Date().toISOString(),
    seq,
