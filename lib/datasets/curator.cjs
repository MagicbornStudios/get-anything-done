...
function classifyEvent(event) {
  ...
  switch (event.kind) {
    ...
    case 'memory':
      // Memory type drives label; fall back to 'feedback' for unknown types
      if (event.memory_type === 'feedback') return 'correction';
      if (event.memory_type === 'project') return 'success';
      if (event.memory_type === 'reference') return 'plain-chat';
      return 'feedback' in event ? 'correction' : 'plain-chat';
    default:
      return 'plain-chat';
  }
}
...
/**
 * Walk .planning/memory/*.md and yield classified events.
 *
 * Added: task GLOBAL-T-252-09 (memory-sync corpus integration).
 * Each memory file is a Markdown doc with YAML frontmatter (name, description, type).
 * Yielded as kind='memory' so downstream consumers can filter or label separately.
 * The SFT/DPO pipeline treats these as standing operator rules — high-signal training data.
 */
function* walkMemory(planningDir) {
  const memDir = path.join(planningDir, 'memory');
  if (!fs.existsSync(memDir)) return;
  let files;
  try { files = fs.readdirSync(memDir).filter((f) => f.endsWith('.md') && f !== 'INDEX.md'); } catch { return; }
...
async function runCuration({ projects = [], log, dryRun = false }) {
  ...
  try {
    ...
    for (const planningDir of planningDirs) {
      // Walk all source types — task GLOBAL-T-252-09 adds walkMemory
      const generators = [
        walkTranscripts(planningDir),
        walkTraces(planningDir),
        walkGadLog(planningDir),
        walkMemory(planningDir),  // .planning/memory/*.md (synced from ~/.claude memory)
      ];
