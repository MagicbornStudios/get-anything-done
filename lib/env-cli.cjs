...
function createEnvCli(deps = {}) {
  ...
  // ---------- get ----------
  async function getCmd({ keyName, projectid, version, scope, passphrase }) {
    await guarded(
  ...
  // ---------- set ----------
  async function setCmd({ keyName, projectid, provider, scope, scopeBag, passphrase }) {
    await guarded(
      ...
      async () => {
        ...
        await withPass({ passphrase }, async (pass) => {
          await store.set({
            ...
            provider: provider || '',
            scope: scope || '',
            scopeBag: scopeBag || undefined,
            passphrase: pass,
            forcePassphrase: !!passphrase,
          });
          stderr.write(`Stored ${keyName} (v1) for project ${projectid}${scopeBag ? ` / ${scopeBag}` : ''}.\n`);
        });
      },
    );
  }
  ...
  // ---------- list ----------
  async function listCmd({ projectid, json, scope, scopeChain, passphrase }) {
    await guarded(
      ...
      async () => {
        await withPass({ passphrase }, async (pass) => {
          let rows;
          try {
            if (scopeChain && Array.isArray(scopeChain) && scopeChain.length > 0) {
              // Chain mode (60-07b): walk most-specific → least-specific,
              // surfacing scopeBag + shadows[] per row. Useful for the UI
              // and for `gad env list --scope-chain species,eval,` style
              // command lines where empty segments mean the planning bag.
              rows = await store.listChain({
                projectId: projectid,
                scopeChain,
                passphrase: pass,
                forcePassphrase: !!passphrase,
  ...
  // ---------- rotate ----------
  async function rotateCmd({ keyName, projectid, graceDays, scope, passphrase }) {
    await guarded(
      { stderr, exit, ctxBuilder: () => ({ projectId: projectid, keyName }) },
      async () => {
        ...
        await withPass({ passphrase }, async (pass) => {
          ...
          stderr.write(
            `Rotated ${keyName} for project ${projectid}${scope ? ` / ${scope}` : ''}: v${oldVersion} → v${newVersion} (grace ${g}d).\n`,
          );
        });
      },
    );
  ...
  // ---------- revoke ----------
  async function revokeCmd({ keyName, projectid, version, scope, force, passphrase }) {
    await guarded(
      { stderr, exit, ctxBuilder: () => ({ projectId: projectid, keyName, version }) },
      async () => {
        // Renamed local from `scope` to `subject` (60-07b): the scope flag
        // now threads through to store.revoke, and shadowing it here was
        // silently dropping the bag selector — store.revoke received
        // undefined and operated on the planning bag regardless of --scope.
        if (!force) {
          const subject = version === undefined
            ? `ALL versions of '${keyName}'`
            : `version ${version} of '${keyName}'`;
          const ok = await confirm(`Revoke ${subject} for project ${projectid}${scope ? ` / ${scope}` : ''}? [y/N] `);
          if (!ok) {
        ...
        await withPass({ passphrase }, async (pass) => {
          ...
          const subject = version === undefined ? 'all versions' : `version ${version}`;
          stderr.write(`Revoked ${subject} of ${keyName} for project ${projectid}${scope ? ` / ${scope}` : ''}.\n`);
        });
      },
    );
  }
...
function renderListTable(rows) {
  // Base columns per byok-design §7.3: KEY VERSION PROVIDER SCOPE LAST
  // ROTATED. 60-07b adds optional BAG (the resolved envelope's scope id)
  // and SHADOWS (parent scopes that contributed an entry hidden by a
  // more-specific bag). They appear only when at least one row carries
  // that metadata so single-bag listings stay visually clean.
  const showBag = rows.some((r) => Object.prototype.hasOwnProperty.call(r, 'scopeBag'));
  const showShadows = rows.some((r) => Array.isArray(r.shadows) && r.shadows.length > 0);
  const headers = ['KEY', 'VERSION', 'PROVIDER', 'SCOPE'];
  if (showBag) headers.push('BAG');
  if (showShadows) headers.push('SHADOWS');
  headers.push('LAST ROTATED');
  const data = rows.map((r) => {
    const row = [r.keyName, String(r.currentVersion), r.provider || '', r.scope || ''];
    if (showBag) row.push(r.scopeBag === null || r.scopeBag === undefined ? '-' : String(r.scopeBag));
    if (showShadows) {
      const sh = Array.isArray(r.shadows) ? r.shadows : [];
      row.push(sh.length === 0 ? '' : sh.map((s) => (s === null ? '(planning)' : String(s))).join(','));
    }
    row.push(shortDate(r.lastRotated));
    return row;
  });
