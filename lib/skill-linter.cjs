 ...
 * <id>` (single-skill health). Can also run in manual release checks if
 * the project opts in, but the default install does NOT block on lint failures
 * per operator intent ("should not be blocking, but easily found").
 *
