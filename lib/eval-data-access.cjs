...
function listGenerations(projectId, speciesName) {
  ...
  for (const e of entries) {
    ...
    const manifest = readGenerationManifest(dir);
    generations.push({
      version: e.name,
      versionNumber: vNum,
      dir,
...
// ---------------------------------------------------------------------------
// Generation manifest (task 44-39)
// ---------------------------------------------------------------------------
//
// Per-generation MANIFEST.json carries the publish lifecycle:
//   { status: "draft" | "published" | "unlisted",
//     publishedAt: ISO timestamp | null,
//     publishedBy: string | null }
//
// Status defaults to "draft" when the file is missing. The marketplace shows
// rows with status === "published". The legacy "isPublished" boolean (derived
// from public/playable copy existence) is kept for back-compat but the manifest
// is the source of truth — the file copy is a side effect of publishing.

const VALID_STATUSES = new Set(['draft', 'published', 'unlisted']);

function manifestPath(generationDir) {
  return path.join(generationDir, 'MANIFEST.json');
}

function readGenerationManifest(generationDir) {
  const p = manifestPath(generationDir);
  if (!fs.existsSync(p)) {
...
function writeGenerationManifest(generationDir, manifest) {
  if (!fs.existsSync(generationDir)) {
    throw new Error(`Generation directory does not exist: ${generationDir}`);
  }
  const status = VALID_STATUSES.has(manifest.status) ? manifest.status : 'draft';
  const out = {
    status,
    publishedAt: manifest.publishedAt || null,
    publishedBy: manifest.publishedBy || null,
  };
  fs.writeFileSync(
    manifestPath(generationDir),
...
/**
 * Set a generation's publish status. Writes MANIFEST.json. Caller is
 * responsible for the public/playable side effect (copy/remove build).
 */
function setGenerationStatus(projectId, speciesName, version, status, publishedBy) {
  ...
  if (!fs.existsSync(dir)) {
    throw new Error(`Generation not found: ${projectId}/${speciesName}/${version}`);
  }
  const prev = readGenerationManifest(dir);
  const next = {
    status,
    // Stamp publishedAt only on transition into "published"
    publishedAt:
      status === 'published'
        ? prev.status === 'published' && prev.publishedAt
          ? prev.publishedAt
          : new Date().toISOString()
        : prev.publishedAt,
    publishedBy:
      status === 'published'
        ? publishedBy || prev.publishedBy || 'anonymous'
        : prev.publishedBy,
  };
  return writeGenerationManifest(dir, next);
}
