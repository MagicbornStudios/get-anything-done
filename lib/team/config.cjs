'use strict';
/**
 * lib/team/config.cjs - team config.json read/write + runtime resolution.
 *
 * Schema v2 (adds workers_spec):
 *   {
 *     workers: 3,                     // number of workers (redundant with workers_spec length)
 *     roles: ["executor", ...],       // legacy flat list, still honored as fallback
 *     workers_spec: [                 // NEW in M3.4 - preferred
 *       { id: "w1", role: "executor", lane: null,      runtime: "claude-code", runtime_cmd: null },
 *       { id: "w2", role: "reviewer", lane: "backend", runtime: "codex-cli",   runtime_cmd: null },
 *     ],
 *     runtime: "claude-code",         // team default, fallback when workers_spec entry omits runtime
 *     runtime_cmd: null,              // team default CLI override
 *     autopause_threshold: 20,
 *     tick_ms: 2000,
 *     runtime_tick_overrides: {       // optional per-runtime worker poll overrides
 *       "gemini-cli": 8000,
 *       "codex-cli": 2000
 *     },
 *     created_at, supervisor_pid
 *   }
 */

const { readJsonSafe, writeJson } = require('./io.cjs');
const { configPath, agentProfilePath } = require('./paths.cjs');

const DEFAULT_TICK_MS = 2000;
const DEFAULT_RUNTIME_TICK_OVERRIDES = Object.freeze({
  'gemini-cli': 8000,
  'codex-cli': 2000,
});

function defaultRuntimeCmd(runtime) {
  if (runtime === 'codex-cli') return 'codex exec';
  if (runtime === 'cursor-cli') return 'node scripts/gad-cursor-trial.mjs -- --print --output-format json';
  if (runtime === 'gemini-cli') return 'gemini';
  if (runtime === 'opencode') return 'node scripts/gad-opencode-trial.mjs -- run --format json';
  return 'claude -p';
}

function maybeWrapSessionTelemetry(runtime, runtimeCmd) {
  if (process.env.GAD_SESSION_TELEMETRY !== '1') return runtimeCmd;
  if (runtime === 'codex-cli') {
    if (/\bcodex-session-emit\.cjs\b/i.test(runtimeCmd)) return runtimeCmd;
    return `node scripts/codex-session-emit.cjs -- ${runtimeCmd}`;
  }
  if (runtime === 'cursor-cli') {
    if (/\bcursor-session-emit\.cjs\b/i.test(runtimeCmd)) return runtimeCmd;
    return `node scripts/cursor-session-emit.cjs -- ${runtimeCmd}`;
  }
  return runtimeCmd;
}

function readConfig(baseDir) {
  return readJsonSafe(configPath(baseDir), null);
}

function writeConfig(baseDir, cfg) {
  writeJson(configPath(baseDir), cfg);
}

function workerSpec(cfg, id) {
  if (cfg && Array.isArray(cfg.workers_spec)) {
    const hit = cfg.workers_spec.find((worker) => worker.id === id);
    if (hit) return hit;
  }
  const n = Number(String(id).replace(/^w/, '')) || 1;
  const role = (cfg && cfg.roles && cfg.roles[n - 1]) || 'executor';
  return { id, role, lane: null, runtime: (cfg && cfg.runtime) || 'claude-code', runtime_cmd: null };
}

// ── Agent-profile presets (GLOBAL-D-406) ─────────────────────────────────────
// A reusable preset at .planning/team/agent-profiles/<id>.json bundling
// { soul, model, role, skills }. A worker_spec references it via
// `agent_profile`; inline worker_spec fields override the preset.

function resolveAgentProfile(baseDir, profileId) {
  if (!baseDir || !profileId) return null;
  return readJsonSafe(agentProfilePath(baseDir, profileId), null);
}

const RUNTIMES_TAKING_MODEL_FLAG = new Set(['claude-code', 'kael-local']);

/**
 * Merge a worker's effective config from inline spec > agent_profile > default.
 * Returns { runtime, model, soul, role, skills, runtime_cmd }. `baseDir` is
 * required to resolve the agent_profile preset; without it, only inline +
 * cfg-default fields resolve (backward compatible).
 */
function resolveWorkerProfile(baseDir, cfg, workerId) {
  const spec = workerId ? workerSpec(cfg, workerId) : null;
  const profile = (spec && spec.agent_profile)
    ? resolveAgentProfile(baseDir, spec.agent_profile)
    : null;

  const pick = (field) => {
    if (spec && spec[field] != null && spec[field] !== '') return spec[field];
    if (profile && profile[field] != null && profile[field] !== '') return profile[field];
    return null;
  };

  const runtime = (spec && spec.runtime) || (cfg && cfg.runtime) || 'claude-code';
  const skills = Array.isArray(spec && spec.skills)
    ? spec.skills
    : (Array.isArray(profile && profile.skills) ? profile.skills : []);

  return {
    runtime,
    model: pick('model'),
    soul: pick('soul'),
    role: pick('role') || (spec && spec.role) || 'executor',
    skills,
    runtime_cmd: (spec && spec.runtime_cmd) || null,
  };
}

function injectModelFlag(runtime, cmd, model) {
  if (!model || !RUNTIMES_TAKING_MODEL_FLAG.has(runtime)) return cmd;
  if (/(^|\s)--model(\s|=)/.test(cmd)) return cmd; // already specifies a model
  return `${cmd} --model ${model}`;
}

function resolveRuntimeCmd(cfg, workerId, baseDir = null) {
  const spec = workerId ? workerSpec(cfg, workerId) : null;
  const runtime = (spec && spec.runtime) || (cfg && cfg.runtime) || 'claude-code';
  let rawCmd = process.env.GAD_TEAM_RUNTIME_CMD
    || (spec && spec.runtime_cmd)
    || (cfg && cfg.runtime_cmd)
    || defaultRuntimeCmd(runtime);
  // Inject --model from the resolved agent-profile when the worker_spec didn't
  // bake one into runtime_cmd. Fixes the dropped-model bug for UI-created
  // claude-code/kael-local workers (GLOBAL-D-406). No-op when baseDir is null.
  if (!process.env.GAD_TEAM_RUNTIME_CMD && (!spec || !spec.runtime_cmd) && baseDir) {
    const { model } = resolveWorkerProfile(baseDir, cfg, workerId);
    rawCmd = injectModelFlag(runtime, rawCmd, model);
  }
  return maybeWrapSessionTelemetry(runtime, rawCmd);
}

function resolveRuntime(cfg, workerId) {
  const spec = workerId ? workerSpec(cfg, workerId) : null;
  return (spec && spec.runtime) || (cfg && cfg.runtime) || 'claude-code';
}

function normalizeRuntimeTickOverrides(cfg) {
  const merged = {
    ...DEFAULT_RUNTIME_TICK_OVERRIDES,
    ...((cfg && cfg.runtime_tick_overrides && typeof cfg.runtime_tick_overrides === 'object')
      ? cfg.runtime_tick_overrides
      : {}),
  };
  const normalized = {};
  for (const [runtime, rawMs] of Object.entries(merged)) {
    const ms = Number(rawMs);
    if (Number.isFinite(ms) && ms > 0) normalized[runtime] = ms;
  }
  return normalized;
}

function resolveTickMs(cfg, workerId) {
  if (process.env.GAD_TEAM_TICK_MS) {
    return Number(process.env.GAD_TEAM_TICK_MS) || DEFAULT_TICK_MS;
  }
  const runtime = workerId ? resolveRuntime(cfg, workerId) : null;
  const runtimeOverrides = normalizeRuntimeTickOverrides(cfg);
  if (runtime && runtimeOverrides[runtime]) return runtimeOverrides[runtime];
  return Number((cfg && cfg.tick_ms) || DEFAULT_TICK_MS) || DEFAULT_TICK_MS;
}

module.exports = {
  DEFAULT_TICK_MS,
  DEFAULT_RUNTIME_TICK_OVERRIDES,
  defaultRuntimeCmd,
  maybeWrapSessionTelemetry,
  readConfig,
  writeConfig,
  workerSpec,
  resolveAgentProfile,
  resolveWorkerProfile,
  resolveRuntimeCmd,
  resolveRuntime,
  resolveTickMs,
  normalizeRuntimeTickOverrides,
};
