...
// agent-profiles/ = reusable per-worker presets (soul/model/role/skills),
// distinct from profiles/ which are whole-team rosters. See GLOBAL-D-406.
const AGENT_PROFILES_DIR = path.join(TEAM_DIR, 'agent-profiles');
...
function agentProfilesRoot(baseDir) { return path.join(baseDir, AGENT_PROFILES_DIR); }
...
function agentProfilePath(baseDir, id) { return path.join(agentProfilesRoot(baseDir), `${id}.json`); }

module.exports = {
  TEAM_DIR, WORKERS_DIR, PROFILES_DIR, AGENT_PROFILES_DIR, LOCKS_DIR,
  teamRoot, workersRoot, profilesRoot, agentProfilesRoot, locksRoot,
  workerDir, configPath, supervisorLog, dispatchSeqPath,
  stopFlagPath, workerLog, workerStatus, workerMailbox, workerOutDir,
  profilePath, agentProfilePath,
};
