...
// not per-minute throttle. When matched, parkRuntime should use a much
// longer cooldown (operator's account-resets-tomorrow scenario).
...
const HARD_CAP_COOLDOWN_MS = 4 * 60 * 60 * 1000;

function suggestedCooldownMs(stderr, stdout) {
  return isHardUsageCap(stderr, stdout) ? HARD_CAP_COOLDOWN_MS : DEFAULT_COOLDOWN_MS;
}
