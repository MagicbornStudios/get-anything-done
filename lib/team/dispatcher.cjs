...
const { isHandoffExhausted, isHandoffExhaustedForRuntimes, parkingEnabled } = require('./rate-limit.cjs');
...
async function runDaemon(baseDir, _opts) {
  ...
  // G1: Log parking state on startup so operator can see it at a glance.
  const parkingState = parkingEnabled(process.env) ? 'enabled' : 'disabled';
  process.stderr.write(`[gad-team] runtime parking: ${parkingState}\n`);
  logWrite({ kind: 'parking-state', parking_enabled: parkingEnabled(process.env), env_var: 'GAD_ENABLE_RUNTIME_PARKING', note: 'parking is opt-OUT; set GAD_ENABLE_RUNTIME_PARKING=0 to disable' });
