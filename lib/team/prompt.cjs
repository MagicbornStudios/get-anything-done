...
function composePrompt(work, { workerId, lane, runtime, skillsScope } = {}) {
  ...
  // Parse phase id from work.ref. Handoffs: "h-...-<projectid>-<phase>". Tasks: "<phase>-<n>".
  const phaseFromRef = (() => {
    const ref = String(work.ref || '');
    if (work.kind === 'handoff') {
      const m = ref.match(/-([\d.]+)$/);
      return m ? m[1] : null;
    }
  ...
  const sweepProjectId = work.projectid || '<projectid>';
  const sweepPhase = phaseFromRef || '<N>';
  ...
  lines.push(`- After stamping done, run \`gad phases sweep --phase ${sweepPhase} --auto-close --projectid ${sweepProjectId}\` if this completes the phase.`);
  return lines.join('\n');
}
