...
// Lazy-load notifications to avoid circular deps. Safe: notifications/index.cjs
// has no dependency on lib/team/. The require is deferred to call-time so
// startup cost is zero when notifications are not triggered.
function tryCreateNotification(opts) {
  try {
    const { createNotification } = require('../notifications/index.cjs');
    return createNotification(opts);
  } catch { /* non-fatal — notification substrate may not be present yet */ }
}
...
function checkAndLogRestart(baseDir, projectid, reason) {
  ...
  if (count >= STORM_MAX) {
    ...
    // Surface to the global notification substrate so statusline + SessionStart
    // consumers can surface the condition without reading the alarm file directly.
    const nowPlus1h = new Date(Date.now() + 60 * 60 * 1000).toISOString();
    tryCreateNotification({
      severity: 'error',
      source: 'dispatcher',
      title: 'Dispatcher restart-storm',
      message: `Dispatcher restart-stormed ${count} times in ${STORM_WINDOW_MS / 60000} min${projectid ? ` (project: ${projectid})` : ''}. Manual investigation required.`,
      expires_at: nowPlus1h,
      actions: [{ label: 'Stop dispatcher', command: 'gad team dispatcher stop' }],
      fingerprint: 'dispatcher-restart-storm',
      _baseDir: baseDir,
    });
