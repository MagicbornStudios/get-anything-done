...
// Build a short soul+skills preamble prepended to the worker's prompt pipe
// when the resolved agent-profile set GAD_WORKER_SOUL / GAD_WORKER_SKILLS
// (GLOBAL-D-406). Returns a bash segment ending in `; ` or '' when neither is
// set. Full soul-body / skill-body assembly is a follow-up; this is the
// pointer-level injection so the runtime knows its voice + which skills to load.
function buildProfilePreamble() {
  ...
  const parts = ['[GAD WORKER PROFILE]'];
  if (soul) parts.push(`soul=${soul} — adopt this voice/role for this work.`);
  if (skills) parts.push(`load these skills before relevant work: ${skills}.`);
  const esc = parts.join(' ').replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  return `printf '%s\\n\\n' "${esc}"; `;
}
