...
async function runWorker(baseDir, id) {
  ...
  // Per decision gad-272: liveness is inferred from log.jsonl mtime, not
  // written heartbeats. Worker-loop cadence != codex subprocess liveness,
  // so writing last_heartbeat every tick is misleading and wasted I/O.
