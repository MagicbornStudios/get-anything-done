...
const { parkRuntime, isParked, nextAvailableRuntime } = require('./rate-limit.cjs');
...
async function runWorker(baseDir, id) {
  ...
  while (!stopRequested(baseDir, id)) {
    ...
    if (popped) {
      ...
      // Skip self-claim if our primary runtime is parked in cooldown.
    ...
    let result;
    try {
      result = await runSubprocess(baseDir, id, runtimeCmd, promptFile, logWrite);
    } finally {
    ...
    // Rate-limit auto-switch (operator directive 2026-05-03):
    // If the runtime returned a rate-limit pattern in stderr/stdout, park it
    // in cooldown, unclaim the handoff back to open/ so a different runtime
    // can grab it, and continue the worker loop. Don't markFailed — failure
    // here is a runtime-availability blip, not a task failure.
    if (result.rate_limited && work.kind === 'handoff' && handoffsLib) {
      ...
      // Unclaim — move back to open/ for another runtime to pick up.
      try {
        if (typeof handoffsLib.unclaimHandoff === 'function') {
          handoffsLib.unclaimHandoff({ baseDir, id: work.ref });
        } else {
          // Fallback: rename the file directly. handoffsLib may expose a
          // path resolver; if not, attempt the conventional layout.
          const claimedPath = path.join(baseDir, '.planning', 'handoffs', 'claimed', `${work.ref}.md`);
          const openPath = path.join(baseDir, '.planning', 'handoffs', 'open', `${work.ref}.md`);
          if (fs.existsSync(claimedPath)) fs.renameSync(claimedPath, openPath);
        }
      ...
      // Sleep an extra beat to give the cooldown registry a moment.
      await sleep(tickMs);
      continue;
    }
