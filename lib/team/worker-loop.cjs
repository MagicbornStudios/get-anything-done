...
const { readConfig, resolveRuntime, resolveRuntimeCmd, resolveTickMs, workerSpec, resolveWorkerProfile } = require('./config.cjs');
...
const { applyCodexModelRouting } = require('./codex-model-routing.cjs');
...
const { recordAccountQuotaState } = require('./accounts-registry.cjs');
...
/**
 * Persist quota observations onto the account that just hit a rate-limit (110-06).
 * Silent no-op if account info is missing — workers shouldn't crash on attribution gaps.
 */
function persistAccountQuotaObservation({ baseDir, account, classification, result, logWrite }) {
  if (!account || !account.label || !account.provider) return;
  try {
    const obs = classification || classifyRuntimeError(result && result.stderr, result && result.code, null);
    const resetAt = obs.cooldown_until ? new Date(obs.cooldown_until).toISOString() : null;
    const updated = recordAccountQuotaState(baseDir, account.provider, account.label, {
      status: 'rate-limited',
      last_error: obs.reason_text || 'rate-limited',
      reset_at: resetAt,
      error_class: obs.class || null,
      reason_text: obs.reason_text || null,
      cooldown_ms: obs.cooldown_ms || null,
    });
    if (updated && logWrite) {
      logWrite({
        kind: 'account-quota-state-recorded',
    ...
    if (logWrite) logWrite({ kind: 'account-quota-state-error', error: err.message });
  }
}
...
function handleRateLimitedHandoff({
  ...
  // 110-06: persist quota observation onto the account that just failed,
  // BEFORE rotating away from it. account-poller will flip rate-limited→active
  // once reset_at passes.
  persistAccountQuotaObservation({ baseDir, account: activeAccount, classification, result, logWrite });
 ...
 * @returns {{ action: 'requeue'|'surface'|'backoff'|'restart'|'noop', classification: object }}
 */
function handleClassifiedFailure(classification, ctx) {
  const { baseDir, runtime, work, handoffsLib, workerId, logWrite, result, activeAccount } = ctx;
  ...
  // 110-06: for quota classes, also persist observation onto the account
  // record so external tooling (account-poller, UI dashboards, rotate
  // heuristics) can see the cooldown without re-scanning worker logs.
  if (cls === 'quota_soft' || cls === 'quota_hard_cap') {
    persistAccountQuotaObservation({ baseDir, account: activeAccount, classification, result, logWrite });
  }
  ...
  if (cls === 'quota_soft' || cls === 'quota_hard_cap') {
    // Just-try-it: unclaim the handoff so another runtime can attempt it.
    // No cooldown/parking state is written — the per-handoff retry budget
    // (isHandoffExhausted) is the only block. Next tick, a different worker
    // (probably a fallback runtime because sortHandoffsForPickup orders by
    // preference) self-claims and tries. After 3 rate-limit unclaims the
    // handoff is blocked from further dispatch until operator intervenes.
...
async function runWorker(baseDir, id) {
  ...
  const runtimeCmd = resolveRuntimeCmd(cfg, id, baseDir);
  const tickMs = resolveTickMs(cfg, id);

  process.env.GAD_TEAM_WORKER_ID = id;
  process.env.GAD_AGENT_NAME = `team-${id}`;

  // Surface the resolved agent-profile (soul/model/skills) to the spawned
  // runtime via env so subprocess.cjs can build a soul+skills prompt preamble
  // and downstream tooling can scope skills per worker (GLOBAL-D-406).
  const wp = resolveWorkerProfile(baseDir, cfg, id);
  if (wp.soul) process.env.GAD_WORKER_SOUL = wp.soul; else delete process.env.GAD_WORKER_SOUL;
  if (wp.model) process.env.GAD_WORKER_MODEL = wp.model; else delete process.env.GAD_WORKER_MODEL;
  if (wp.skills && wp.skills.length) process.env.GAD_WORKER_SKILLS = wp.skills.join(','); else delete process.env.GAD_WORKER_SKILLS;
  ...
  while (!stopRequested(baseDir, id)) {
    ...
    if (popped) {
      ...
      if (m.kind === 'handoff' && handoffsLib) {
        try {
          ...
          // Fix 273-15: when claimHandoff throws ALREADY_CLAIMED the handoff
          // was pre-claimed by the dispatcher (claimed_by: handoff-dispatcher)
          // and routed here via mailbox. The handoff body is NOT gone — it
          // lives in claimed/ — so attempt a direct read before giving up.
          // readHandoff searches all buckets (open/claimed/closed).
          if (err && err.code === 'ALREADY_CLAIMED') {
            try {
              const loaded = handoffsLib.readHandoff({ baseDir, id: m.ref });
              // Only proceed if the handoff is still in claimed/ (not closed).
              // A closed handoff means another worker already completed it —
              // that is a true collision and we should skip.
              if (loaded && loaded.bucket === 'claimed') {
                body = loaded.body || '';
                frontmatter = loaded.frontmatter || {};
                logWrite({ kind: 'claim-via-dispatcher-preempt', ref: m.ref, claimed_by: (loaded.frontmatter && loaded.frontmatter.claimed_by) || 'unknown' });
              } else {
                claimFailed = true;
                logWrite({ kind: 'claim-error', ref: m.ref, error: `handoff in bucket '${loaded && loaded.bucket}', cannot process`, original_error: err.message });
              }
            } catch (readErr) {
              claimFailed = true;
              logWrite({ kind: 'claim-error', ref: m.ref, error: err.message, read_error: readErr.message });
            }
          } else {
            claimFailed = true;
    ...
    let effectiveRuntimeCmd = runtimeCmd;
    if (runtime === 'codex-cli') {
      try {
        const routed = applyCodexModelRouting(runtimeCmd, work.frontmatter || {}, { baseDir });
        effectiveRuntimeCmd = routed.runtimeCmd;
        if (routed.routing) {
          logWrite({
            kind: 'runtime-model-routed',
            runtime,
            ref: work.ref,
            source: routed.routing.source,
            route_key: routed.routing.routeKey,
            tier: routed.routing.tier,
            selected_model: routed.routing.selectedModel,
            fast_model: routed.routing.fastModel,
            strong_model: routed.routing.strongModel,
            available_models: routed.routing.availableModels,
          });
        }
      } catch (err) {
        logWrite({
          kind: 'runtime-model-routing-error',
          runtime,
          ref: work.ref,
          error: err.message,
        });
    ...
    try {
      ...
      for (;;) {
        result = await runSubprocess(
          ...
          { ...subprocessOpts, runtimeId: runtime },
        );
    ...
    logWrite({
      ...
      tokens_input: telemetry.tokens_in,
      tokens_output: telemetry.tokens_out,
      tokens_total: Number.isFinite(telemetry.tokens_in) && Number.isFinite(telemetry.tokens_out)
