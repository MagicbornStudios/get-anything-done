...
const { parkRuntime, isParked, getActiveRuntimeAccount, rotateRuntimeAccount, suggestedCooldownMs, isHandoffExhausted, loadCooldown, loadFallbackChain } = require('./rate-limit.cjs');
...
function handleRateLimitedHandoff({
  ...
  // Hours-long usage caps ("You've hit your usage limit") get a much
  // longer cooldown than per-minute throttle ("exhausted your capacity").
  const cooldownMs = result
    ? suggestedCooldownMs(result.stderr, result.stdout)
    : undefined;
  const cooldown = parkRuntime(baseDir, runtime, cooldownMs);
  logWrite({
...
async function runWorker(baseDir, id) {
  ...
  function trySelfClaim() {
    ...
    try {
      ...
      // Build fallback context: any runtime currently parked + per-runtime
      // fallback chain. When codex is parked, a gemini worker picking up a
      // codex-preferred handoff is the right move (auto-switch behavior).
      const cooldownReg = (typeof loadCooldown === 'function') ? loadCooldown(baseDir) : {};
      const parkedRuntimes = new Set(
        Object.entries(cooldownReg)
          .filter(([, v]) => v && v.until && Date.now() < v.until)
          .map(([k]) => k)
      );
      const fallbackChains = {};
      for (const r of parkedRuntimes) {
        if (typeof loadFallbackChain === 'function') {
          fallbackChains[r] = loadFallbackChain(baseDir, r);
        }
      }
      const compat = open.filter((handoff) => {
        ...
        return agentDetect.isHandoffCompatible(rp, runtime, { parkedRuntimes, fallbackChains }) && matchesLane(lane, hlane);
      });
