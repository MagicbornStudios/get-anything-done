...
const MACHINE_CLASSES = [
  {
    ...
    where_used: ['validation', 'routing', 'stale-task detection', 'path/file checks'],
    disk_mb: 0,
    usage_level: 'none'
  },
  {
    ...
    where_used: ['classification', 'routing', 'DSL fill', 'summaries'],
    disk_mb: 650,
    usage_level: 'warm'
  },
  {
    ...
    where_used: ['structured DSL generation', 'task refinement', 'simple code explanation', 'local code review'],
    disk_mb: 2250,
    usage_level: 'hot'
  },
  {
    ...
    where_used: ['local code reasoning', 'code review', 'refactors', 'architecture', 'debugging', 'tests'],
    disk_mb: 7000,
    usage_level: 'hot'
  }
];
 ...
 *   where_used: string[],
 *   disk_mb: number|null,
 *   usage_level: 'hot'|'warm'|'cold'|'none',
 *   warnings: string[]
 * }}
 */
function recommendTeamProfile(machineProfile, opts = {}) {
  ...
  return {
    ...
    disk_mb: selected.disk_mb,
    usage_level: selected.usage_level,
    warnings
  };
}
...
if (require.main === module) {
  const samples = [
    { label: 'Tiny', profile: { ram_free_gb: 2.3, disk_free_gb: 50 } },
    { label: 'Medium', profile: { ram_free_gb: 12, disk_free_gb: 50 } }
  ];

  for (const sample of samples) {
    const recommendation = recommendTeamProfile(sample.profile);
