...
function applyDefaultSubcommandInjectors() {
  ...
  // gad ask "<bare sentence>" → gad ask llm "<sentence>" --backend modal --soul "Kael of Tarro"
  // Routes the desktop-assistant default: Kael soul, Dr. Stein model, Modal backend.
  // Detection: first positional after `ask` is NOT a recognized subcommand AND
  // is a non-flag string with whitespace OR length >= 4 (heuristic: "looks like
  // a sentence, not a typo of a subcommand"). Sets MODAL_VLLM_URL +
  // MODAL_VLLM_MODEL env defaults so callModal in _ask-llm.cjs picks them up.
  // Explicit `gad ask llm "..."` path is untouched (first positional matches
  // a known subcommand → this injector no-ops).
  (function injectBareAskLlmDefault() {
    const a = process.argv;
    const i = a.indexOf('ask');
    if (i === -1) return;
    const first = a[i + 1];
    if (first === undefined) return;
    if (typeof first !== 'string') return;
    if (first.startsWith('-')) return;
    const known = new Set(['llm', 'env', 'byok', 'todo', 'decision', 'text', 'help']);
    if (known.has(first)) return;
    // Heuristic for "sentence-like": whitespace OR length >= 4.
    // (Avoids hijacking accidental typos of subcommands like "lm" → 2 chars no whitespace.)
    if (!(first.includes(' ') || first.length >= 4)) return;

    // Backend routing: GAD_KAEL_BACKEND env var picks one of:
    //   modal     — self-hosted Modal vLLM endpoint (Dr. Stein adapter today)
    //   anthropic — Anthropic API direct (claude-sonnet-4-6 default per soul_routes.toml)
    //   auto      — _ask-llm.cjs detects: modal > gateway > direct
    // Default: modal (operator desktop default). Generic pattern is
    // GAD_<SOUL_SLUG_UPPER>_BACKEND so future souls extend the same way.
    const preferredBackend = (process.env.GAD_KAEL_BACKEND
      || process.env.GAD_ASK_DEFAULT_BACKEND
      || 'modal').toLowerCase();

    // Only set MODAL_VLLM_URL/MODEL defaults when actually routing to modal.
    // Otherwise we'd misleadingly populate env vars that the chosen backend
    // doesn't use.
    if (preferredBackend === 'modal' || preferredBackend === 'auto') {
      if (!process.env.MODAL_VLLM_URL) {
        process.env.MODAL_VLLM_URL =
          'https://b2gdevs--slm-learning-vllm-vllmengine-chat-completions.modal.run';
      }
      if (!process.env.MODAL_VLLM_MODEL) {
        process.env.MODAL_VLLM_MODEL = 'dr-stein-cli-v2';
      }
    }

    // Map our preferred-backend vocabulary onto _ask-llm.cjs's accepted values.
    // anthropic → direct (uses ANTHROPIC_API_KEY in _ask-llm.cjs callDirect path).
    const backendValue = preferredBackend === 'anthropic' ? 'direct' : preferredBackend;

    // If the user explicitly passed --backend on the command line, leave them alone.
    const hasBackendFlag = a.some(
      (t, idx) => idx > i && (t === '--backend' || (typeof t === 'string' && t.startsWith('--backend='))),
    );
    const hasSoulFlag = a.some(
      (t, idx) => idx > i && (t === '--soul' || (typeof t === 'string' && t.startsWith('--soul='))),
    );

    // Splice `llm` between `ask` and the bare sentence.
    a.splice(i + 1, 0, 'llm');
    // Append our defaults after the sentence. Citty parses flags positionally
    // but ignores their order relative to positionals, so appending at the
    // end is safe.
    if (!hasBackendFlag) a.push('--backend', backendValue);
    if (!hasSoulFlag) a.push('--soul', 'Kael of Tarro');
  })();
}
