...
function applyDefaultSubcommandInjectors() {
  ...
  (function injectBareAskLlmDefault() {
    ...
    // Default MODAL_VLLM_URL + MODAL_VLLM_MODEL when unset (operator desktop default).
    ...
    // Forced backend: modal (since URL is now guaranteed-set above).
    // If the user already passed --backend, leave their choice alone.
    ...
    // end is safe.
    if (!hasBackendFlag) a.push('--backend', 'modal');
