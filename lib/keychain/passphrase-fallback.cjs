 ...
 *     a newline).
 *   - Also throws PASSPHRASE_REQUIRED_NO_TTY when GAD_NO_TTY_PROMPT=1 is
 *     set in the environment (60-07b regression fix). Long-running headless
 *     hosts such as the Next.js dev server inherit the parent terminal's
 *     TTY, so isTTY would otherwise be true and a blocking prompt would
 *     open inside an HTTP request handler. Setting GAD_NO_TTY_PROMPT=1
 *     at process start forces the structured error so the surrounding UI
 *     can prompt out-of-band and replay the call with `passphrase: ...`.
 */
...
function readPassphraseFromTty(prompt, opts) {
  ...
  if (process.env.GAD_NO_TTY_PROMPT === '1' || !stdin.isTTY) {
    return Promise.reject(
      new SecretsStoreError(
        'PASSPHRASE_REQUIRED_NO_TTY',
        process.env.GAD_NO_TTY_PROMPT === '1'
          ? 'GAD_NO_TTY_PROMPT=1 is set; pass passphrase explicitly via API'
