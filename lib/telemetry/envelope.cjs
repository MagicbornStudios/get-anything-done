...
// Phase 145 follow-up (Phase 148 prep): inferred content_type axis. Not a
// schema bump — optional field, populated by lib/telemetry/content-type.cjs.
...
const OPTIONAL_FIELDS = ['task_id', 'handoff_id', 'model', 'parent_id', 'agent_id', 'content_type'];
...
function validateEnvelope(env) {
  ...
  if (
    env.content_type !== undefined &&
    env.content_type !== null &&
    !VALID_CONTENT_TYPES.has(env.content_type)
  ) {
    errors.push(`content_type not in ${[...VALID_CONTENT_TYPES].join('|')}: got ${env.content_type}`);
  }
