...
async function* iterEnvelopes(rootDir, sinceMs = 0) {
  ...
  for (const workerId of listWorkerDirs(rootDir)) {
    ...
    for (const { name, tsMs } of listPromptFiles(outDir)) {
      ...
      const content = { text, source_file: name };

      // Worker prompt files are mostly handoff-prompt text (planning
      // artefacts). If the embedded prompt body references concrete
      // file paths that match a non-planning rule, prefer that signal.
      const inferred = deriveContentType({ content });
      const contentType = inferred !== 'meta' ? inferred : 'planning';
