...
function rowToEnvelopes(row) {
  ...
  if (row.type === 'tool_use') {
    ...
    // Row-level content_type — classify on the inputs (which carry the
    // file path). Both call + result envelopes share the same type so
    // tool_call and its tool_result agree about the domain.
    const rowType = deriveContentType({ content: callContent });
  ...
  if (row.type === 'assistant_response' || row.type === 'assistant_reasoning') {
    let role = row.type === 'assistant_response' ? 'response' : 'reasoning';
    if (row.kind === 'kael_proposal' || row.kind === 'kael_action_status' || row.kind === 'kael_action') {
      role = 'kael_action';
    }
    const text = row.content && typeof row.content.text === 'string' ? row.content.text : '';
    const tag = role === 'response' ? 'response' : (role === 'reasoning' ? 'reasoning' : role);
    const id = deriveEnvelopeId(`${SOURCE_TAG}|seq-${seqKey}|${tag}`);
    const content = { text };
    if (row.kind) content.kind = row.kind;
...
function tryMake(fields) {
  try {
    const withContentType = {
      ...fields,
      content_type: fields.content_type || deriveContentType(fields),
    };
    return { ok: true, envelope: makeEnvelope(withContentType) };
  } catch (err) {
