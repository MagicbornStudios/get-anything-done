...
async function writeJsonl(outPath, asyncIter) {
  ...
  try {
    for await (const { env } of asyncIter) {
      ...
      count += 1;
      histogram[env.role] = (histogram[env.role] || 0) + 1;
    }
  } finally {
    fs.closeSync(fd);
  }
  return { rowCount: count, roleHistogram: histogram };
}

async function writeManifest({ outDir, dataFile, since, until, rowCount, roleHistogram, monorepoSha, gadSha }) {
...
async function runExport(opts) {
  ...
  const stats = await writeJsonl(dataPath, iterAllEnvelopes(rootDir, sinceMs, opts.adapters));
