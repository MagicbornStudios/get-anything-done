...
const REGISTRY = [
  ...
  },
  {
    ...
    default: 90000,
    scope: 'project',
    description: 'Age threshold in ms before a claimed handoff is reclaimed by the periodic sweeper (default 90 s)',
    validate: (v) => Number.isInteger(v) && v > 0,
  },
  {
    ...
    default: 30000,
    scope: 'project',
    description: 'Age threshold in ms before a worker heartbeat is considered stale for immediate reclaim (default 30 s)',
    validate: (v) => Number.isInteger(v) && v > 0,
  },
  {
    ...
    default: 90,
    scope: 'project',
    description: 'Age threshold in seconds after which status alarm fires for a stale claim (default 90 s)',
    validate: (v) => Number.isInteger(v) && v > 0,
  },
  {
  ...
  // ---------------------------------------------------------------------------
  // RTK (Rust Token Killer) — phase 260, 2026-05-18
  // Token-saving proxy installed at ~/.local/bin/rtk.exe (Windows) or
  // ~/.local/bin/rtk (Unix). Activated via PreToolUse hook in
  // ~/.claude/settings.json. These settings let gad surfaces (desk panels,
  // CLI, metrics rollup) discover whether rtk is in use + where the binary
  // lives without scraping settings.json.
  // ---------------------------------------------------------------------------
  {
    key: 'workflow.rtk.enabled',
    type: 'boolean',
    default: false,
    envVar: 'GAD_RTK_ENABLED',
    scope: 'user',
    description: 'When true, gad surfaces treat rtk as active (telemetry collection, savings panels, rollup column populated). Set independently of whether the PreToolUse hook is installed — this is gad-side intent, not hook reachability. Default OFF (opt-in per phase 260).',
    validate: (v) => typeof v === 'boolean',
  },
  {
    key: 'workflow.rtk.binary_path',
    type: 'string|null',
    default: null,
    envVar: 'GAD_RTK_BINARY_PATH',
    scope: 'user',
    description: 'Absolute path to the rtk binary. null = auto-detect from PATH (which rtk / where.exe rtk). Used by rtk-tracker.cjs to invoke `rtk gain -f json` and by the desk savings panel. Set if you have multiple rtk variants installed (reachingforthejack/rtk collision) and want to lock onto one.',
    validate: (v) => v === null || (typeof v === 'string' && v.length > 0),
  },
];
