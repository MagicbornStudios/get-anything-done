...
function collectAttributions(config, baseDir, ctx = {}) {
  ...
  for (const root of roots) {
    const planningDir = path.join(baseDir, root.path, root.planningDir || '.planning');

    // Post-63-53: per-task JSON is the source of truth.
    const tasksDir = path.join(planningDir, 'tasks');
    let jsonHandled = false;
    try {
      const stat = impl.statSync ? impl.statSync(tasksDir) : null;
      if (stat && stat.isDirectory()) {
        jsonHandled = true;
        const files = impl.readdirSync(tasksDir).filter((f) => f.endsWith('.json') && !f.endsWith('.tmp'));
        for (const f of files) {
          let task;
          try { task = JSON.parse(impl.readFileSync(path.join(tasksDir, f), 'utf8')); }
          catch { continue; }
          if (!task || String(task.status || '').toLowerCase() !== 'done') continue;
          if (!task.skill) continue;
          const date = task.updated_at || task.completed || '';
          const skills = String(task.skill).split(',').map((s) => s.trim()).filter(Boolean);
          for (const s of skills) {
            if (SENTINEL_SKILL_VALUES.has(s.toLowerCase())) continue;
            all.push({ skill: s, taskId: String(task.id || ''), projectId: root.id, date });
          }
        }
      }
    ...
    // Legacy XML fallback (pre-migration projects only).
