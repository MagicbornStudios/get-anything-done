...
// ---------------------------------------------------------------------------
// Isolation Forest integration (task 246-08)
// Lazy-loaded so the module stays backward-compatible when isoforest is absent.
// ---------------------------------------------------------------------------

let _isoForestModule = null;
function getIsoForest() {
  if (!_isoForestModule) {
    try { _isoForestModule = require('./isoforest.cjs'); } catch { _isoForestModule = false; }
  }
  return _isoForestModule || null;
}

/**
 * Build numeric feature rows from token-budgets.jsonl entries.
 * Features: [totalTokens, deltaTokens, hourOfDay]
 */
function buildTokenFeatureMatrix(lines) {
  const rows = [];
  for (let i = 1; i < lines.length; i++) {
    const prev = lines[i - 1];
    const curr = lines[i];
    if (!curr.totalTokens || !prev.totalTokens) continue;
    const delta = curr.totalTokens - prev.totalTokens;
    const ts = curr.ts ? new Date(curr.ts).getHours() : 0;
    rows.push([curr.totalTokens, Math.max(0, delta), ts]);
  }
  return rows;
}

/**
 * Detect anomalies using Isolation Forest on token-budget feature matrix.
 * Falls back to rule-based if isoforest module unavailable or insufficient data.
 *
 * @param {object} opts
 * @param {string} opts.baseDir
 * @param {number} [opts.threshold=0.62]   — anomaly score threshold
 * @returns {Promise<{fired:boolean, evidence:object}>}
 */
async function detectWithIsolationForest({ baseDir, threshold = 0.62 } = {}) {
  const IF = getIsoForest();
  if (!IF) {
    return { fired: false, evidence: { reason: 'isoforest module not available; use rule-based detector' } };
  }

  const budgetsPath = path.join(baseDir, '.planning', '.gad-log', 'token-budgets.jsonl');
  const lines = readJsonlLines(budgetsPath);
  if (lines.length < 10) {
    return { fired: false, evidence: { reason: `insufficient data: ${lines.length} entries` } };
  }

  const rows = buildTokenFeatureMatrix(lines);
  if (rows.length < 5) {
    return { fired: false, evidence: { reason: 'insufficient feature rows' } };
  }

  const forest = new IF.IsolationForest({ n_trees: 50, sample_size: Math.min(64, rows.length), threshold });
  forest.fit(rows);

  const predictions = forest.predict(rows);
  ...
  return {
    ...
    evidence: {
      ...
      top_scores: anomalies.slice(0, 5).map((a) => ({
        idx: a.idx,
