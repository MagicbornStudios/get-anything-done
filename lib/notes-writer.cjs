...
function extractOpenQuestions(text) {
  ...
  // Match `## Open questions`, `## Questions`, or `## Open Questions for <...>`
  // Use multi-line anchor; require heading on its own line.
  // Same-line qualifier after "questions" must use [ \t]+ not \s+, otherwise
  // the greedy \s+ eats the blank line + first numbered item.
  const re = /(^|\n)##[ \t]+(?:open[ \t]+)?questions(?:[ \t]+[^\n]*)?[ \t]*\r?\n/i;
  const match = src.match(re);
  if (!match) return '';
  // match.index is the position of the `\n` (or 0 if first char). Skip past
  // the entire match so the first numbered item is included — earlier version
  // dropped item 1 because of a slice-off-by-one on CRLF input.
