 ...
 * compatible.
 */
function isHandoffCompatible(handoffRuntime, currentRuntime, opts) {
  const pref = String(handoffRuntime || '').trim().toLowerCase();
  const curr = String(currentRuntime || '').trim().toLowerCase();
  if (!pref || pref === 'any') return true;
  if (!curr) return false;
  if (pref === curr) return true;
  // Fallback: if the preferred runtime is currently parked AND the current
  // worker's runtime is in its fallback chain, accept the handoff. This is
  // the auto-switch path — when codex hits a usage cap, gemini/opencode
  // workers pick up codex-preferred handoffs and keep the queue moving.
  if (opts && opts.parkedRuntimes && opts.fallbackChains) {
    const parked = opts.parkedRuntimes;
    if (parked.has ? parked.has(pref) : Array.isArray(parked) ? parked.includes(pref) : false) {
      const chain = opts.fallbackChains[pref] || [];
      if (chain.includes(curr)) return true;
    }
  }
  return false;
}
