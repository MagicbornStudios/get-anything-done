...
try {
  ...
  // Tolerate ALL require errors — bun --compile bundles a stub for missing
  // modules that throws a generic Error (not MODULE_NOT_FOUND), so the prior
  // strict code check rejected the fallback path in the compiled exe and
  // killed startup. Token-drain incident 2026-05-09 surfaced this when the
  // rebuilt gad.exe failed to launch.
  void e;
