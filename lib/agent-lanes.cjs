...
// Named agents (non-ghost) that are >7 days old and have no outstanding task
// claims (or whose claims are all done/cancelled) are also pruned on write.
// This catches workers like codex-cli-default-0001, cursor-default-0001, w1
// that the ghost compactor skips because they don't match GHOST_AGENT_RE.
const NAMED_STALE_PRUNE_AFTER_MS = 7 * 24 * 60 * 60 * 1000; // 7 days
...
function compactLaneState(state) {
  ...
  const kept = (state.agents || []).filter((agent) => {
    ...
    // Drop named agents (non-ghost) that are stale beyond NAMED_STALE_PRUNE_AFTER_MS
    // and have no task/phase claims remaining.  These accumulate when workers
    // complete all their tasks but are never explicitly released.
    if (!GHOST_AGENT_RE.test(agent.agentId)) {
      const lastSeen = agent.lastSeenAt ? Date.parse(agent.lastSeenAt) : 0;
      const age = Number.isFinite(lastSeen) ? now - lastSeen : Infinity;
      if (age > NAMED_STALE_PRUNE_AFTER_MS) return false;
    }
    return true;
  });
