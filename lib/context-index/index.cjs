...
const { buildIndex, queryIndex, readNdjson } = require('./search.cjs');

const ALL_SOURCES = ['gad-log', 'state-log', 'git-log', 'planning-artifacts'];
 ...
 * @returns {Promise<Array<{id,text,source,ts,sessionId,score,snippet}>>}
 */
async function query(queryText, opts = {}) {
  ...
  let results = queryIndex(iPath, queryText, {
    topK: opts.topK || 10,
...
module.exports = { ingest, query, summarize, rebuild };
