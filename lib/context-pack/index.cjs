...
const { query: queryPlanningIndex } = require('./planning-index.cjs');
...
function buildContextPack({ cid, taskId, repoRoot, planningDirs }) {
  ...
  let refs = [];
  try {
    refs = queryPlanningIndex(target, {
      planningDir: resolvedPlanningDirs[0],
      repoRoot,
      topK: 10,
    });
  } catch {
  ...
  if (refs.length === 0) {
    const legacyRefs = planningGrep(repoRoot, target).sort().reverse().slice(0, 10);
    ...
    for (const ref of refs) {
      lines.push(`- \`${ref.type}:${ref.sourceId}\`${ref.snippet ? ` — ${ref.snippet}` : ''}`);
    }
  }
  lines.push('');
