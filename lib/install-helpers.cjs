...
// Markers used by filterHookEntries to identify GAD-installed hooks
// (so install / uninstall is idempotent). gad-trace-hook is the
// PreToolUse/PostToolUse handler; gad-stop-hook is the Stop /
// SubagentStop handler added in phase 145 (T-145-04).
const GAD_HOOK_MARKER = 'gad-trace-hook';
const GAD_STOP_HOOK_MARKER = 'gad-stop-hook';
const GAD_SESSION_END_HOOK_MARKER = 'claude-session-end-hook';
const GAD_HOOK_MARKERS = [GAD_HOOK_MARKER, GAD_STOP_HOOK_MARKER, GAD_SESSION_END_HOOK_MARKER];
...
function isPackagedExecutableRuntime() {
  if (process.env.GAD_PACKAGED_EXECUTABLE || process.env.GAD_PACKAGED_ROOT) return true;
  // Fallback: env vars can be missing when the installed shim didn't inject
  // them (stale install, direct invocation, Windows Start-menu launch).
  // If process.execPath is itself the packaged gad binary, treat as packaged.
  try {
    const base = path.basename(process.execPath).toLowerCase();
    if (base === 'gad.exe' || base === 'gad' || base === 'get-anything-done.exe') return true;
  } catch { /* noop */ }
  return false;
}
...
function maybeGenerateDailyTip({ teachings, scriptDir }) {
  ...
  // Packaged gad binaries have no Node interpreter on PATH guarantee, and
  // process.execPath is the gad exe itself — spawning it with a .mjs path
  // would be parsed as an unknown subcommand. Daily-tip generation runs
  // via CI (see .github/workflows/daily-tip.yml) and `gad tip generate`
  // from a dev checkout; skipping here is the correct behaviour.
  if (isPackagedExecutableRuntime()) return;
...
module.exports = {
  GAD_HOOK_MARKER, GAD_STOP_HOOK_MARKER, GAD_SESSION_END_HOOK_MARKER, GAD_HOOK_MARKERS,
