 ...
 *   { workerId, runtime, handoffId, ts, inputTokens, outputTokens, totalTokens, rateLimited, durationMs }
 */
function parseWorkerLogTokens(workerId, filePath) {
  ...
  let accumulatedTokens = null;
  let rateLimitedCurrent = false;

  function flushHandoff(completeTs, durationMs, tokenFields) {
    if (!activeHandoffId) return;
    const inputTokens = tokenFields && tokenFields.inputTokens != null ? tokenFields.inputTokens : null;
    const outputTokens = tokenFields && tokenFields.outputTokens != null ? tokenFields.outputTokens : null;
    const totalTokens = tokenFields && tokenFields.totalTokens != null
  ...
  for (const entry of lines) {
    ...
    switch (entry.kind) {
      ...
      case 'work-start': {
        ...
        if (activeHandoffId) flushHandoff(entry.ts, null, null);
      ...
      case 'subproc-stderr': {
        ...
        if (expectingTokenCount) {
          ...
          if (count !== null) {
            accumulatedTokens = (accumulatedTokens || 0) + count;
          }
          expectingTokenCount = false;
          break;
        }
      ...
      case 'work-complete': {
        ...
        const inputTokens = normalizeTokenField(entry.tokens_input != null ? entry.tokens_input : entry.tokens_in);
        const outputTokens = normalizeTokenField(entry.tokens_output != null ? entry.tokens_output : entry.tokens_out);
        const totalTokens = normalizeTokenField(entry.tokens_total);
  ...
  if (activeHandoffId) flushHandoff(null, null, null);
