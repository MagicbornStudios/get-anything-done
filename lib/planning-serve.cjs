...
const PLANNING_APP_WORKSPACE = '@portfolio/planning-app';
...
function resolveWorkspaceRoot(startDir) {
  ...
  for (let i = 0; i < 12; i++) {
    const planningAppPkg = path.join(dir, 'apps', 'planning-app', 'package.json');
    if (fs.existsSync(planningAppPkg)) {
      return { root: dir, planningAppDir: path.join(dir, 'apps', 'planning-app') };
    }
