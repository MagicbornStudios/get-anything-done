...
describe('SPAWN: spawn type consistency', () => {
  test('no "First, read agent .md" workaround pattern remains', () => {
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        ...
        const hasWorkaround = content.includes('First, read ~/.claude/agents/gad-');
        assert.ok(
          !hasWorkaround,
  ...
  test('diagnose-issues uses gad-debugger (not general-purpose)', () => {
    const content = fs.readFileSync(
      path.join(WORKFLOWS_DIR, 'diagnose-issues.md'), 'utf-8'
    );
    assert.ok(
      content.includes('subagent_type="gad-debugger"'),
      'diagnose-issues should spawn gad-debugger, not general-purpose'
    );
  });
...
describe('CLAUDEMD: CLAUDE.md compliance enforcement', () => {
  test('gad-plan-checker has Dimension 10: CLAUDE.md Compliance', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gad-plan-checker.md'), 'utf-8');
    assert.ok(
      content.includes('Dimension 10: CLAUDE.md Compliance'),
      'gad-plan-checker must have Dimension 10 for CLAUDE.md compliance checking'
    );
    assert.ok(
      content.includes('claude_md_compliance'),
      'gad-plan-checker must use claude_md_compliance as dimension identifier'
    );
  });

  test('gad-phase-researcher has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gad-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'gad-phase-researcher must enforce CLAUDE.md directives during research'
    );
    assert.ok(
      content.includes('Project Constraints (from CLAUDE.md)'),
      'gad-phase-researcher must output a Project Constraints section from CLAUDE.md'
    );
  });

  test('gad-executor has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gad-executor.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'gad-executor must enforce CLAUDE.md directives during execution'
    );
    assert.ok(
      content.includes('CLAUDE.md rule — it takes precedence over plan instructions'),
      'gad-executor must specify CLAUDE.md precedence over plan instructions'
    );
  });

  test('all three agents read CLAUDE.md in project_context', () => {
    const agents = ['gad-plan-checker', 'gad-phase-researcher', 'gad-executor'];
...
describe('VERIFY: data-flow trace, environment audit, and behavioral spot-checks', () => {
  test('gad-verifier has Step 4b: Data-Flow Trace', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gad-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 4b: Data-Flow Trace'),
      'gad-verifier must have Step 4b for data-flow tracing'
    );
    assert.ok(
      content.includes('HOLLOW'),
      'gad-verifier must define HOLLOW status for wired-but-disconnected artifacts'
    );
    assert.ok(
      content.includes('DISCONNECTED'),
      'gad-verifier must define DISCONNECTED status for missing data sources'
    );
  });

  test('gad-verifier has Step 7b: Behavioral Spot-Checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gad-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 7b: Behavioral Spot-Checks'),
      'gad-verifier must have Step 7b for behavioral spot-checks'
    );
    assert.ok(
      content.includes('SKIP'),
      'gad-verifier spot-checks must support SKIP status for untestable items'
    );
  });

  test('gad-verifier VERIFICATION.md template includes data-flow and spot-check sections', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gad-verifier.md'), 'utf-8');
    assert.ok(
  ...
  test('gad-verifier success criteria include data-flow and spot-checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gad-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Data-flow trace (Level 4)'),
  ...
  test('gad-phase-researcher has Step 2.6: Environment Availability Audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gad-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('Step 2.6: Environment Availability Audit'),
      'gad-phase-researcher must have Step 2.6 for environment availability auditing'
    );
    assert.ok(
      content.includes('Environment Availability'),
      'gad-phase-researcher must include Environment Availability section in RESEARCH.md template'
    );
  });

  test('gad-phase-researcher success criteria include environment audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gad-phase-researcher.md'), 'utf-8');
    assert.ok(
...
describe('DISCUSS: discussion log generation', () => {
  ...
  test('discussion-log template exists', () => {
    const templatePath = path.join(__dirname, '..', 'templates', 'discussion-log.md');
    assert.ok(
...
describe('COMPAT: agents must not use runtime-specific frontmatter keys', () => {
  ...
  const AGENTS_WITH_WRITE = ['gad-executor', 'gad-debugger'];
