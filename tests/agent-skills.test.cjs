...
describe('agent-skills command', () => {
  ...
  test('returns empty when no config exists', () => {
    ...
    const result = runGsdTools(['agent-skills', 'gad-executor'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
  ...
  test('returns empty for unconfigured agent type', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'gad-executor': ['skills/test-skill'],
      },
    });
    const result = runGsdTools(['agent-skills', 'gad-planner'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.strictEqual(result.output, '');
  });
  ...
  test('handles multiple skill paths', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'gad-executor': ['skills/skill-a', 'skills/skill-b'],
      },
    });
  ...
  test('warns for nonexistent skill path but does not error', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'gad-executor': ['skills/nonexistent'],
      },
    });
...
describe('config-set agent_skills', () => {
  ...
  test('can set agent_skills via dot notation', () => {
    const result = runGsdTools(
      ['config-set', 'agent_skills.gad-executor', '["skills/my-skill"]'],
      tmpDir,
    ...
    assert.deepStrictEqual(
      config.agent_skills['gad-executor'],
      ['skills/my-skill'],
