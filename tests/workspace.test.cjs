 ...
 * GSD Workspace Tests
 *
 * Tests for /gad:new-workspace, /gad:list-workspaces, /gad:remove-workspace
 * init functions and integration with gad-tools routing.
 */
...
const { detectChildRepos } = require('../lib/init.cjs');
...
describe('detectChildRepos', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('gad-ws-test-');
  });
...
// ─── cmdInitNewWorkspace via gad-tools ──────────────────────────────────────
...
// ─── cmdInitListWorkspaces via gad-tools ────────────────────────────────────
...
describe('init list-workspaces', () => {
  ...
  test('finds workspaces with WORKSPACE.md', () => {
    const wsBase = path.join(tmpDir, 'gad-workspaces');
...
// ─── cmdInitRemoveWorkspace via gad-tools ───────────────────────────────────
...
describe('workspace worktree integration', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('gad-ws-integ-');
...
describe('workspace command files', () => {
  ...
  test('new-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/new-workspace.md'), 'utf8');
    assert.ok(content.includes('name: gad:new-workspace'));
  ...
  test('list-workspaces command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('name: gad:list-workspaces'));
    assert.ok(content.includes('workflows/list-workspaces.md'));
  });

  test('remove-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('name: gad:remove-workspace'));
    assert.ok(content.includes('workflows/remove-workspace.md'));
  });

  test('new-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'get-anything-done/workflows/new-workspace.md'), 'utf8');
  ...
  test('list-workspaces workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'get-anything-done/workflows/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('init list-workspaces'));
  });

  test('remove-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'get-anything-done/workflows/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('init remove-workspace'));
    assert.ok(content.includes('git worktree remove'));
  });
});

// ─── Routing in gad-tools ───────────────────────────────────────────────────

describe('workspace routing in gad-tools', () => {
  test('init new-workspace is routed correctly', () => {
    const toolsContent = fs.readFileSync(
      path.join(__dirname, '..', 'bin', 'gad-tools.cjs'),
      'utf8'
    );
