...
describe('OpenCode agent conversion (isAgent: true)', () => {
  test('keeps name: field for agents', () => {
    ...
    assert.ok(frontmatter.includes('name: gad-executor'), 'name: should be preserved for agents');
  });
  ...
  test('strips skills: array', () => {
    ...
    assert.ok(!frontmatter.includes('gad-executor-workflow'), 'skill entries should be stripped');
  });
...
Use \${PHASE} in shell examples.
    ...
    assert.ok(frontmatter.includes('name: gad-codebase-mapper'), 'keeps name');
    ...
    assert.ok(!frontmatter.includes('gad-mapper-workflow'), 'drops skills list items');
