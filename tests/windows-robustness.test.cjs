 ...
 *
 * Regression tests for: https://github.com/gad-build/get-anything-done/issues/1343
 */
...
describe('@file: handoff in workflows', () => {
  test('all workflows calling gad-tools init have @file: handler', () => {
    const workflowFiles = fs.readdirSync(WORKFLOWS_DIR)
      .filter(f => f.endsWith('.md'));
    ...
    for (const wf of workflowFiles) {
      ...
      // Check if this workflow calls gad-tools.cjs init
      if (/INIT=\$\(node.*gad-tools.*\binit\b/.test(content)) {
    ...
    assert.strictEqual(
      missing.length, 0,
      `Workflows calling gad-tools init without @file: handler (large output will be truncated):\n  ${missing.join('\n  ')}`
    );
  });
});
