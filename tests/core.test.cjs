...
describe('loadConfig', () => {
  ...
  test('returns model_overrides when present (REG-01)', () => {
    writeConfig({ model_overrides: { 'gad-executor': 'opus' } });
    const config = loadConfig(tmpDir);
    assert.deepStrictEqual(config.model_overrides, { 'gad-executor': 'opus' });
  });
...
describe('resolveModelInternal', () => {
  ...
  describe('model profile structural validation', () => {
    test('all known agents resolve to a valid string for each profile', () => {
      const knownAgents = ['gad-planner', 'gad-executor', 'gad-phase-researcher', 'gad-codebase-mapper'];
  ...
  describe('override precedence', () => {
    test('per-agent override takes precedence over profile', () => {
      writeConfig({
        ...
        model_overrides: { 'gad-executor': 'haiku' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'gad-executor'), 'haiku');
    });

    test('opus override resolves to opus', () => {
      writeConfig({
        model_overrides: { 'gad-executor': 'opus' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'gad-executor'), 'opus');
    });
    ...
    test('agents not in override fall back to profile', () => {
      ...
      // gad-planner not overridden, should use quality profile -> opus
      assert.strictEqual(resolveModelInternal(tmpDir, 'gad-planner'), 'opus');
    });
  });
  ...
  describe('edge cases', () => {
    test('returns sonnet for unknown agent type', () => {
      writeConfig({ model_profile: 'balanced' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'gad-nonexistent'), 'sonnet');
    });
    ...
    test('defaults to balanced profile when model_profile missing', () => {
      writeConfig({});
      // balanced profile, gad-planner -> opus
      assert.strictEqual(resolveModelInternal(tmpDir, 'gad-planner'), 'opus');
    });
  });
  ...
  describe('resolve_model_ids: "omit"', () => {
    test('returns empty string for known agents', () => {
      ...
      assert.strictEqual(resolveModelInternal(tmpDir, 'gad-planner'), '');
    });

    test('returns empty string for unknown agents', () => {
      writeConfig({ resolve_model_ids: 'omit' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'gad-nonexistent'), '');
    });

    test('still respects model_overrides even when omit', () => {
      writeConfig({
        resolve_model_ids: 'omit',
        model_overrides: { 'gad-planner': 'openai/gpt-5.4' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'gad-planner'), 'openai/gpt-5.4');
    });
...
describe('safeReadFile', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gad-core-test-'));
  });
...
describe('stale hook filter', () => {
  test('filter should only match gad-prefixed .js files', () => {
    const files = [
      'gad-check-update.js',
      ...
      'gad-check-update.js.bak',         // backup file
      'README.md',                       // non-js file
    ];

    const gsdFilter = f => f.startsWith('gad-') && f.endsWith('.js');
    ...
    assert.deepStrictEqual(filtered, [
      ...
      'gad-statusline.js',
      'gad-workflow-guard.js',
    ], 'should only include gad-prefixed .js files');
...
describe('stale hook path', () => {
  test('gad-check-update.js checks configDir/hooks/ where hooks are actually installed (#1421)', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'hooks', 'gad-check-update.js'), 'utf-8'
    );
    // Hooks are installed at configDir/hooks/ (e.g. ~/.claude/hooks/),
    // not configDir/get-anything-done/hooks/ which doesn't exist (#1421)
    assert.ok(
...
describe('shared cache directory (#1421)', () => {
  test('gad-check-update.js writes cache to shared ~/.cache/gsd/ directory', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'hooks', 'gad-check-update.js'), 'utf-8'
    );
  ...
  test('gad-statusline.js checks shared cache first, falls back to legacy (#1421)', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'hooks', 'gad-statusline.js'), 'utf-8'
    );
    // Statusline must check the shared cache path first
    assert.ok(
      content.includes("path.join(homeDir, '.cache', 'gsd', 'gad-update-check.json')"),
      'statusline must check shared cache at ~/.cache/gsd/gad-update-check.json'
    );
    // Must fall back to legacy runtime-specific cache for backward compat
    assert.ok(
      content.includes("path.join(claudeDir, 'cache', 'gad-update-check.json')"),
      'statusline must fall back to legacy cache at claudeDir/cache/gad-update-check.json'
    );
...
describe('resolveWorktreeRoot with linked worktree .planning/', () => {
  ...
  function initBareGitRepo() {
    const dir = normalizePath(fs.mkdtempSync(path.join(os.tmpdir(), 'gad-wt-main-')));
  ...
  test('returns linked worktree cwd when it has its own .planning/', () => {
    ...
    worktreeDir = normalizePath(fs.mkdtempSync(path.join(os.tmpdir(), 'gad-wt-linked-')));
...
describe('monorepo worktree CWD preservation', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gad-monorepo-wt-'));
  });
...
describe('withPlanningLock', () => {
  const { withPlanningLock, planningDir } = require('../lib/core.cjs');
  let tmpDir;
...
describe('detectSubRepos', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gad-detect-test-'));
  });
...
describe('loadConfig sub_repos auto-sync', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gad-sync-test-'));
...
describe('findProjectRoot', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gad-root-test-'));
  });
...
describe('reapStaleTempFiles', () => {
  test('removes stale gad-*.json files older than maxAgeMs', () => {
    const tmpDir = os.tmpdir();
    const stalePath = path.join(tmpDir, `gad-reap-test-${Date.now()}.json`);
    fs.writeFileSync(stalePath, '{}');
    // Set mtime to 10 minutes ago
    const oldTime = new Date(Date.now() - 10 * 60 * 1000);
    fs.utimesSync(stalePath, oldTime, oldTime);

    reapStaleTempFiles('gad-reap-test-', { maxAgeMs: 5 * 60 * 1000 });

    assert.ok(!fs.existsSync(stalePath), 'stale file should be removed');
  });

  test('preserves fresh gad-*.json files', () => {
    const tmpDir = os.tmpdir();
    const freshPath = path.join(tmpDir, `gad-reap-fresh-${Date.now()}.json`);
    fs.writeFileSync(freshPath, '{}');

    reapStaleTempFiles('gad-reap-fresh-', { maxAgeMs: 5 * 60 * 1000 });
  ...
  test('removes stale temp directories when present', () => {
    const tmpDir = os.tmpdir();
    const staleDir = fs.mkdtempSync(path.join(tmpDir, 'gad-reap-dir-'));
    fs.writeFileSync(path.join(staleDir, 'data.jsonl'), 'test');
    // Set mtime to 10 minutes ago
    const oldTime = new Date(Date.now() - 10 * 60 * 1000);
    fs.utimesSync(staleDir, oldTime, oldTime);

    reapStaleTempFiles('gad-reap-dir-', { maxAgeMs: 5 * 60 * 1000 });
  ...
  test('does not throw on empty or missing prefix matches', () => {
    assert.doesNotThrow(() => {
      reapStaleTempFiles('gad-nonexistent-prefix-xyz-', { maxAgeMs: 0 });
    });
  });
});
...
describe('planningDir', () => {
  ...
  beforeEach(() => {
    ...
    savedWorkstream = process.env.GAD_WORKSTREAM;
    delete process.env.GSD_PROJECT;
    delete process.env.GAD_WORKSTREAM;
  });

  afterEach(() => {
    if (savedProject !== undefined) process.env.GSD_PROJECT = savedProject;
    else delete process.env.GSD_PROJECT;
    if (savedWorkstream !== undefined) process.env.GAD_WORKSTREAM = savedWorkstream;
    else delete process.env.GAD_WORKSTREAM;
  });
