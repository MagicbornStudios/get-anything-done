 ...
 * - gad-security-auditor agent frontmatter and structure
...
// ─── 1. Agent frontmatter — gad-security-auditor.md ─────────────────────────

describe('SECURE: gad-security-auditor agent', () => {
  const agentPath = path.join(AGENTS_DIR, 'gad-security-auditor.md');

  test('agent file exists', () => {
    assert.ok(
      fs.existsSync(agentPath),
      'gad-security-auditor.md must exist in agents/'
    );
  });
  ...
  test('name is gad-security-auditor', () => {
    const content = fs.readFileSync(agentPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: gad-security-auditor'),
      'name must be gad-security-auditor'
    );
  });
...
describe('SECURE: secure-phase command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in commands/'
    );
  });

  test('has valid frontmatter with name gad:secure-phase', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: gad:secure-phase'),
      'name must be gad:secure-phase'
    );
  });
...
describe('SECURE: secure-phase workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in get-anything-done/workflows/'
    );
  });

  test('contains gad-security-auditor reference', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
      content.includes('gad-security-auditor'),
      'must reference gad-security-auditor agent'
    );
  });
...
describe('SECURE: SECURITY.md template', () => {
  ...
  test('template exists', () => {
    assert.ok(
      ...
      'SECURITY.md must exist in get-anything-done/templates/'
    );
  });
...
describe('SECURE: config.json security defaults', () => {
  ...
  test('config template exists', () => {
    assert.ok(
      ...
      'config.json must exist in get-anything-done/templates/'
    );
  });
...
describe('SECURE: VALIDATION.md security columns', () => {
  ...
  test('VALIDATION.md template exists', () => {
    assert.ok(
      ...
      'VALIDATION.md must exist in get-anything-done/templates/'
    );
  });
