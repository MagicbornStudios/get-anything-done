...
const commandPath = path.join(repoRoot, 'commands', 'forensics.md');
const workflowPath = path.join(repoRoot, 'workflows', 'forensics.md');

describe('forensics command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/forensics.md should exist');
  });

  test('command has correct frontmatter', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: gad:forensics'), 'should have correct command name');
...
describe('forensics fixture-based tests', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gad-forensics-test-'));
  });
