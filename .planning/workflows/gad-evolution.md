...
participants:
  skills: [create-proto-skill, gad-evolution-validator, gad-skill-creator]
  agents: [default]
...
is agentic (create-proto-skill drafts in bulk with per-candidate
checkpoints per decision gad-171, then gad-evolution-validator writes
VALIDATION.md advisorily). Stage 3 is human — review and decide
...
flowchart TD
  ...
  B --> C[create-proto-skill bulk run]
  C --> D[write PROVENANCE.md lock marker]
  D --> E[draft SKILL.md in dot-agent format]
  E --> F[gad evolution validate -> VALIDATION.md advisory]
  F --> G[human review SKILL.md + VALIDATION.md]
  G --> H{promote?}
  H -->|yes| I[gad evolution promote -> sdk/skills/<name>/]
  H -->|no| J[gad evolution discard --keepCandidate?]
  I --> K[PROVENANCE.md travels with skill]
  J --> L[candidate returns to queue OR deleted]
```
