<overview>
Git integration for the GAD framework.
</overview>
...
If NO_GIT: Run `git init` silently. GAD projects always get their own repo.
</git_check>
...
```bash
gad-tools commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
gad-tools commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
gad-tools commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
...
For workspaces with separate git repos (e.g., `backend/`, `frontend/`, `shared/`), GAD routes commits to each repo independently.

### Configuration

In `gad-config.toml`, list sub-repo directories under `[[planning.roots]]` (the `.planning/config.json` mirror exposes them as `planning.sub_repos` for compatibility):
...
1. **Auto-detection:** During `/gad:new-project`, directories with their own `.git` folder are detected and offered for selection as sub-repos. On subsequent runs, `loadConfig` auto-syncs the `sub_repos` list with the filesystem — adding newly created repos and removing deleted ones. This means `config.json` may be rewritten automatically when repos change on disk.
2. **File grouping:** Code files are grouped by their sub-repo prefix (e.g., `backend/src/api/users.ts` belongs to the `backend/` repo).
3. **Independent commits:** Each sub-repo receives its own atomic commit via `gad-tools commit-to-subrepo`. File paths are made relative to the sub-repo root before staging.
...
```bash
gad-tools commit-to-subrepo "feat(02-01): add user API" \
