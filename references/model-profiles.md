...
# Or manually in `gad-config.toml` (the `.planning/config.json` mirror is compatibility-only)
{
...
```
1. Read `gad-config.toml` or the generated `.planning/config.json` mirror
...
Per-project default: Set in `gad-config.toml`:
```json
{
  "model_profile": "off"
}
