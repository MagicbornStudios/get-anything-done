...
const startupCmd = defineCommand({
  ...
  run({ args }) {
    ...
    // DAILY SUBAGENTS — surface today's pending subagent dispatches. Default
    // behavior (auto-spawn opt-out) is IMPERATIVE wording that directs the
    // orchestrator to spawn immediately via its runtime's Agent tool. Set
    // gad-config.toml `[subagents] autoSpawn = false` (or env
    // GAD_DAILY_AUTOSPAWN=off) to downgrade to passive "here's what's
    // pending" wording. (D-mod autospawn, 2026-04-19.)
    try {
