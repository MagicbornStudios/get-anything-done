...
// ---------------------------------------------------------------------------
// agents command (2026-04-19) — read-only runtime/agent visibility
// ---------------------------------------------------------------------------
...
const agentsStatusCmd = defineCommand({
  ...
  run({ args }) {
    ...
    // Per-runtime open-handoff count
...
const agentsCmd = defineCommand({
  meta: {
    name: 'agents',
    description: 'Agent / runtime visibility. Read-only inspection; does not spawn.',
  },
  subCommands: {
    status: agentsStatusCmd,
  },
});
...
const worktreePrune = defineCommand({
  ...
  run({ args }) {
    ...
    for (const w of willRemove) {
      try {
        if (w.unregistered) {
          // Not registered with git → just rmdir the on-disk leftover.
...
// Motivated by 2026-04-18 ENOSPC incident: cursor site build blocked by 2 GB
// free of 456 GB total; 17 stale agent worktrees in .claude/worktrees/ that
// `gad worktree list` did not detect. Goal: surface disk pressure + remediate
// from inside the loop so agents catch OOM/ENOSPC before builds fail.
// ---------------------------------------------------------------------------
...
// Bounded recursive size walker: stops as soon as either the entry budget
// or the wall-clock deadline is hit. Acceptance for 63-health-cli is
// "runs under 2s on Windows without hanging du" — Windows fs walks are
// very slow for huge node_modules trees, so we cap aggressively and tag
// the result as truncated when we bail. Returns { bytes, truncated }.
  ...
  while (stack.length) {
    ...
    for (const c of children) {
      ...
      try {
        ...
        // ignore EPERM/ENOENT — disk health is best-effort
      }
    }
  }
...
// Cross-platform disk-free lookup. Prefers fs.statfs (Node 18.15+); falls
// back to Windows `wmic logicaldisk` for older runtimes. Returns
// { totalBytes, freeBytes } or null when unknown.
...
// Hot directories scanned by `gad health disk`. Order matters: caches
// first so they appear at the top of the report.
...
const healthDiskCmd = defineCommand({
  meta: {
    name: 'disk',
    description: 'Report disk free + top hot-directory hogs (bounded; <2s on Windows). 63-health-cli.',
  },
  args: {
  ...
  run({ args }) {
    ...
    if (free) {
      ...
      const lowFree = free.freeBytes < 10 * 1024 * 1024 * 1024; // <10 GB absolute
      const lowPct = free.freeBytes / free.totalBytes < 0.10;   // <10% of volume
      if (lowFree || lowPct) {
...
const healthPruneCmd = defineCommand({
  meta: {
    name: 'prune',
    description: 'Prune stale state safely (worktrees today; caches via `gad health caches --remove`). 63-health-cli.',
  },
  args: {
  ...
  run({ args }) {
    ...
    const worktrees = listAllWorktrees().map(worktreeInfo);
    // Agent-only by default — never auto-prune the main worktree.
    const candidates = worktrees.filter(w => {
    ...
    if (args.json) {
      ...
      if (candidates.length === 0) {
        console.log(`No agent worktrees older than ${args['older-than']}.`);
        return;
      }
...
// Cache directories `gad health caches` knows how to enumerate + clear.
// Conservative list: build outputs that any normal `npm run build` will
// regenerate, plus per-runtime cache dirs the operator can safely re-warm.
...
const healthCachesCmd = defineCommand({
  meta: {
    name: 'caches',
    description: 'List (or remove) reproducible build caches. Default is dry-run. 63-health-cli.',
  },
  args: {
  ...
  run({ args }) {
    ...
    if (args.json) {
      ...
      if (cands.length === 0) {
        console.log('  No cache directories present.');
        return;
      }
      ...
      if (!willRemove) {
        console.log('\n  Dry run. Re-run with --remove to clear these directories.');
        return;
      }
    }
...
const healthCmd = defineCommand({
  meta: {
    name: 'health',
    description: 'Machine health surface — disk pressure, stale-worktree prune, cache enumeration (63-health-cli)',
  },
  subCommands: {
    disk: healthDiskCmd,
    prune: healthPruneCmd,
    caches: healthCachesCmd,
  },
});
