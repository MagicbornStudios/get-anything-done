...
# Always write a tiny wrapper that exec's the canonical pre-push by
# absolute path. Symlinks are unreliable on Windows (MSYS ln -sf silently
# falls back to copy, breaking the dispatcher's sibling-file lookup for
# pre-push.cjs). A wrapper is portable and always finds the .cjs.
