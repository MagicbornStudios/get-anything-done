...
export function PlanningBugsTab({ gadBugs }: { gadBugs: BugRecord[] }) {
  return (
    <div className="space-y-2">
      {gadBugs.map((b) => (
        <div key={b.id} id={b.id} className="flex scroll-mt-24 items-start gap-3 rounded-lg border border-border/50 bg-card/30 p-3">
          <Identified as={`PlanningBugsTabRow-${b.id}`} className="contents">
