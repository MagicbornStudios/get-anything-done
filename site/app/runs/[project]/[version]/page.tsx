...
export default async function RunPage({
  ...
  // Rubric radar data — only render if the run has non-legacy rubric with scored dimensions.
  ...
  const rubricDimensions: RubricDimension[] =
    !rubric?.is_legacy && !rubric?.is_empty && rubricDef?.dimensions
      ...
      : [];
  const hasRubricScores = rubricDimensions.length >= 3 && rubricDimensions.some((d) => d.score != null);
        ...
        <section className="border-b border-border/60 bg-card/20">
          <div className="section-shell">
            ...
            <p className="mt-3 max-w-3xl text-base leading-7 text-muted-foreground">
              ...
              → <code className="rounded bg-card/60 px-1.5 py-0.5 text-xs">human_review_rubric</code>.
              The filled polygon shows the reviewer&apos;s per-dimension scores, 0.0 at center to
              1.0 at the edge. The aggregate score (
              <strong className="text-foreground">
