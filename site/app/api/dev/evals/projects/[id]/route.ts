...
/** GET /api/dev/evals/projects/[id] — full project summary */
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const summary = da.getProjectSummary(id);
    if (!summary) {
      return new Response(
...
/** PUT /api/dev/evals/projects/[id] — update project fields */
export async function PUT(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const da = getDataAccess();
    const updated = da.updateProject(id, body);
...
/** DELETE /api/dev/evals/projects/[id] — archive (soft delete) */
export async function DELETE(_request: Request, { params }: RouteParams) {
  ...
  try {
    const { id } = await params;
    const da = getDataAccess();
    const result = da.archiveProject(id);
