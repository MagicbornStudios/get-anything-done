...
import { RESULTS_REPO, RESULTS_WORKFLOW_TINT } from "@/components/landing/results/results-shared";
...
export function ResultsRunCard({ run }: Props) {
  ...
  return (
    <Card
      className={`border-l-4 ${RESULTS_WORKFLOW_TINT[run.workflow]} transition-colors hover:border-accent/70`}
    >
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3 pt-1 text-xs font-medium">
          ...
          <a
            href={`${RESULTS_REPO}/tree/main/evals/${run.project}/${run.version}`}
            rel="noopener noreferrer"
            target="_blank"
