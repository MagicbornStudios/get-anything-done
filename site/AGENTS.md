...
**Section dev panel** (`SectionDevPanel.tsx`): With dev IDs on (`Alt+I`), each `SiteSection` that opts in shows a gear. The slide-in **Dev Panel** has the accent header, stable **`gad-dev-panel`** id (copy from header), list **sorted by registry depth**, **Tooltip** (opens toward the page, not HoverCard — avoids hover/focus loops inside the transformed slide-in panel) per row for route / id / depth, **Eye** for sticky highlight, row label / **Copy** for clipboard only (**no scroll-into-view** — that path was removed as the source of focus/layout churn). **Message** opens **`DevIdAgentPromptDialog`** (Update / Delete prompts, dictation, copy). Prefer **`sectionBandCid`** on `SiteSection` and outer **`Identified`** bands; inner chrome may use **`register={false}`**.
