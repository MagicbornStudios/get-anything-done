...
**Page-level bands** (top-level page sections): wrap in a top-level `SiteSection cid="<route>-page-site-section"` instead of the deprecated `PageIdentified` (removed 2026-04-14). Page bands get the same hover dev-panel surface, Update/Delete handoff flow, and descendant list as other band-level landmarks. Open-question **category** rows use **`OpenQuestionsCategory-<slug>`** inside `QuestionsCategorySection`; resolved uses **`OpenQuestionsResolvedSection`** inside `QuestionsResolvedSection`.
