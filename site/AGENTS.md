...
**Section dev panel** (`SectionDevPanel.tsx`): With dev IDs on (`Alt+I`), each `SiteSection` shows its panel on **hover/focus** with a fade-in (no gear click). The panel has the accent header, stable **`gad-dev-panel`** id (copy from header), list **sorted by registry depth**, No Radix **HoverCard** / **Tooltip** on list rows (they caused **maximum update depth** inside transformed slide-ins); row label uses a native **`title`** hint and click copies **`data-cid`**, **Eye** for sticky highlight, row label / **Copy** for clipboard only (**no scroll-into-view** — that path was removed as the source of focus/layout churn). **Message** opens **`DevIdAgentPromptDialog`** (Update / Delete prompts, dictation, copy). `SiteSection` auto-registers its section band id; use outer **`Identified`** bands for additional landmarks and `register={false}` for inner chrome.
...
If only a heading or card is tagged, there is **no stable id for the parent band** — the user cannot point at it except by naming an inner id. `SiteSection` auto-provides a stable parent band id (`data-cid` on `<section>`, depth 0). Use an outer `Identified` wrapping the whole shell for additional named landmarks — not only the kicker.

**Page-level bands** (outside any `SiteSection` registry): use **`PageIdentified`** (`components/devid/PageIdentified.tsx`) from route `page.tsx` — it does **not** register in the section dev panel; with dev IDs on, **hover** the band to reveal a **top-left** chip with **`data-cid`** and a **Copy** button (optional **`stableCid`**). Route pages now use the global wrapper path via `MarketingShell`; section internals use `SiteSection` auto-band ids + optional `Identified` landmarks. Open-question **category** rows use **`OpenQuestionsCategory-<slug>`** inside `QuestionsCategorySection`; resolved uses **`OpenQuestionsResolvedSection`** inside `QuestionsResolvedSection`.
