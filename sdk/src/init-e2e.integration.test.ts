 ...
 * sessions for the gad-sdk init workflow.
 *
...
describe.skipIf(!cliAvailable)('E2E: InitRunner.run() full workflow', () => {
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'gad-sdk-init-e2e-'));
  ...
  it('InitRunner.run() bootstraps a project without human intervention', async () => {
    ...
    const initStartEvents = events.filter(e => e.type === GADEventType.InitStart);
    expect(initStartEvents.length).toBe(1);

    const stepCompleteEvents = events.filter(e => e.type === GADEventType.InitStepComplete);
    expect(stepCompleteEvents.length).toBeGreaterThanOrEqual(1);

    // ── Assert: InitComplete event emitted ──
    const initCompleteEvents = events.filter(e => e.type === GADEventType.InitComplete);
