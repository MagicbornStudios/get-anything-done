...
import { GADEventType, type GADEvent, type GADEventBase } from './types.js';
...
function makeBase(overrides: Partial<GADEventBase> = {}): Omit<GADEventBase, 'type'> {
  return {
...
describe('CLITransport', () => {
  ...
  it('formats PhaseStart as bold cyan banner and PhaseComplete with running cost', () => {
    ...
    expect(lines[0]).toBe(`${BOLD}${CYAN}━━━ GAD ► PHASE 01: Authentication ━━━${RESET}`);
  ...
  it('formats MilestoneStart as bold banner and MilestoneComplete with running cost', () => {
    ...
    expect(lines[1]).toBe(`${BOLD}  GAD Milestone — 3 phases${RESET}`);
