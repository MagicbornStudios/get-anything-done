...
import type { ParsedPlan, ContextFiles, GADEvent } from './types.js';
...
describe('InitRunner assembled output', () => {
  ...
  const stubTools: GADTools = {
    ...
    configSet: async () => {},
    commit: async () => {},
  } as unknown as GADTools;

  const stubEventStream: GADEventStream = {
    emitEvent: (_event: GADEvent) => {},
  } as unknown as GADEventStream;
  ...
  describe('buildRoadmapPrompt', () => {
    ...
    it('contains agent definition content', () => {
      // Roadmap prompt loads gad-roadmapper.md
