...
import { GADTools, GADToolsError, resolveGadToolsPath } from './gad-tools.js';
...
describe('GADTools', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `gad-tools-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('exec()', () => {
    ...
    it('throws GADToolsError on non-zero exit code', async () => {
      const scriptPath = await createScript(
        'fail.cjs',
      ...
      try {
        ...
        expect(err).toBeInstanceOf(GADToolsError);
        const gsdErr = err as GADToolsError;
    ...
    it('throws GADToolsError with context when gad-tools.cjs not found', async () => {
      const tools = new GADTools({
        projectDir: tmpDir,
        gadToolsPath: '/nonexistent/path/gad-tools.cjs',
      });

      await expect(tools.exec('state', ['load'])).rejects.toThrow(GADToolsError);
    });
  ...
  describe('initNewProject()', () => {
    ...
    it('propagates errors from gad-tools', async () => {
      const scriptPath = await createScript(
        'init-fail.cjs',
      ...
      await expect(tools.initNewProject()).rejects.toThrow(GADToolsError);
    });
  });

  // ─── resolveGadToolsPath() tests ────────────────────────────────────────

  describe('resolveGadToolsPath()', () => {
    it('returns repo-local path when it exists', async () => {
      const localBinDir = join(tmpDir, '.claude', 'get-anything-done', 'bin');
      await mkdir(localBinDir, { recursive: true });
      await writeFile(join(localBinDir, 'gad-tools.cjs'), '// stub');

      const result = resolveGadToolsPath(tmpDir);
      expect(result).toBe(join(localBinDir, 'gad-tools.cjs'));
    });

    it('falls back to global path when repo-local does not exist', () => {
      const result = resolveGadToolsPath(tmpDir);
      expect(result).toBe(
        join(homedir(), '.claude', 'get-anything-done', 'bin', 'gad-tools.cjs'),
      );
    });
    ...
    it('constructor uses repo-local path when available', async () => {
      const localBinDir = join(tmpDir, '.claude', 'get-anything-done', 'bin');
      await mkdir(localBinDir, { recursive: true });
      const scriptPath = join(localBinDir, 'gad-tools.cjs');
      await writeFile(
        scriptPath,
      ...
      // No explicit gadToolsPath — should auto-resolve to local
      const tools = new GADTools({ projectDir: tmpDir });
