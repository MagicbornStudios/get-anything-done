...
// Mock fs/promises for template reading (InitRunner reads GAD templates)
...
function makeDeps(overrides: Partial<InitRunnerDeps> & { tmpDir: string }): InitRunnerDeps & { events: GADEvent[] } {
...
describe('InitRunner', () => {
  ...
  function createRunner(toolsOverrides: Record<string, unknown> = {}, configOverrides?: Partial<InitRunnerDeps['config']>) {
    ...
    return { runner, tools, eventStream, events: eventStream.events as GADEvent[] };
  }
  ...
  it('run() emits InitStart and InitComplete events', async () => {
    ...
    const startEvents = events.filter(e => e.type === GADEventType.InitStart);
    const completeEvents = events.filter(e => e.type === GADEventType.InitComplete);
  ...
  it('run() emits InitStepStart/Complete for each step', async () => {
    ...
    const stepStarts = events.filter(e => e.type === GADEventType.InitStepStart);
    const stepCompletes = events.filter(e => e.type === GADEventType.InitStepComplete);
  ...
  it('run() emits InitResearchSpawn before research sessions', async () => {
    ...
    const spawnEvents = events.filter(e => e.type === GADEventType.InitResearchSpawn);
  ...
  describe('sdkPromptsDir preference and sanitizer integration', () => {
    ...
    beforeEach(async () => {
      ...
      await writeFile(
        join(sdkPromptsDir, 'agents', 'gad-project-researcher.md'),
        '# Project Researcher Agent\nSDK_HEADLESS_MARKER_RESEARCHER\n',
      );
      await writeFile(
        join(sdkPromptsDir, 'agents', 'gad-research-synthesizer.md'),
        '# Research Synthesizer Agent\nSDK_HEADLESS_MARKER_SYNTHESIZER\n',
      );
      await writeFile(
        join(sdkPromptsDir, 'agents', 'gad-roadmapper.md'),
    ...
    it('readGADFile prefers sdk/prompts/ template over GAD-1 path', async () => {
    ...
    it('readAgentFile prefers sdk/prompts/agents/ over GAD-1 path', async () => {
      const { runner } = createRunnerWithSdkPrompts();

      await runner.run('build a todo app');

      // Research calls (indices 1-4) use gad-project-researcher.md agent def
    ...
    it('readGADFile falls back to GAD-1 when sdk/prompts/ file does not exist', async () => {
      ...
      // falls through to GAD-1 path. If GAD-1 also missing, gets placeholder.
      const projectPrompt = mockRunSession.mock.calls[0]![0] as string;
    ...
    it('readAgentFile falls back to GAD-1 when sdk/prompts/agents/ file does not exist', async () => {
      ...
      // Research prompt uses agent def — not in empty agents dir, falls to GAD-1
      const researchPrompt = mockRunSession.mock.calls[1]![0] as string;
