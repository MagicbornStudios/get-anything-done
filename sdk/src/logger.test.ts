...
import { GADLogger } from './logger.js';
...
describe('GADLogger', () => {
  ...
  it('outputs valid JSON on each log call', () => {
    const logger = new GADLogger({ output, level: 'debug' });
  ...
  it('filters messages below minimum log level', () => {
    const logger = new GADLogger({ output, level: 'warn' });
  ...
  it('defaults to info level filtering', () => {
    const logger = new GADLogger({ output });
  ...
  it('writes to custom output stream', () => {
    ...
    const logger = new GADLogger({ output: customOutput, level: 'debug' });
    logger.info('custom');
