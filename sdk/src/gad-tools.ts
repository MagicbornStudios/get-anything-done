...
export class GADToolsError extends Error {
  constructor(
    message: string,
...
export class GADTools {
  ...
  constructor(opts: {
    ...
    this.gadToolsPath =
      opts.gadToolsPath ?? resolveGadToolsPath(opts.projectDir);
  ...
  private getExecCommandAndArgs(command: string, args: string[], raw = false): { executable: string; args: string[] } {
    ...
    if (this.gadToolsPath.endsWith('.cjs') || this.gadToolsPath.endsWith('.js')) {
      return { executable: 'node', args: [this.gadToolsPath, ...commandArgs] };
    }
    return { executable: this.gadToolsPath, args: commandArgs };
  }
 ...
 */
export function resolveGadToolsPath(projectDir: string): string {
