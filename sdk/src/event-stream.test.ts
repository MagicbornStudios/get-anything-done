...
// ─── SDKMessage → GADEvent mapping tests ─────────────────────────────────────

describe('GADEventStream', () => {
  let stream: GADEventStream;

  beforeEach(() => {
    stream = new GADEventStream();
  });
  ...
  describe('mapSDKMessage', () => {
    it('maps SDKSystemMessage init → SessionInit', () => {
      ...
      expect(event!.type).toBe(GADEventType.SessionInit);

      const init = event as GADSessionInitEvent;
    ...
    it('maps assistant text blocks → AssistantText', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.AssistantText);
      expect((event as GADAssistantTextEvent).text).toBe('Hello world');
    });
    ...
    it('maps assistant tool_use blocks → ToolCall', () => {
      ...
      const event = stream.mapSDKMessage(msg);
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.ToolCall);

      const tc = event as GADToolCallEvent;
    ...
    it('handles multi-block assistant messages (text + tool_use)', () => {
      ...
      stream.on('event', (e: GADEvent) => events.push(e));
      ...
      expect(events[0]!.type).toBe(GADEventType.AssistantText);
      expect(events[1]!.type).toBe(GADEventType.ToolCall);
    });

    it('maps SDKResultSuccess → SessionComplete', () => {
      const event = stream.mapSDKMessage(makeResultSuccess());
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.SessionComplete);

      const complete = event as GADSessionCompleteEvent;
    ...
    it('maps SDKResultError → SessionError', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.SessionError);

      const err = event as GADSessionErrorEvent;
    ...
    it('maps SDKToolProgressMessage → ToolProgress', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.ToolProgress);

      const tp = event as GADToolProgressEvent;
    ...
    it('maps SDKToolUseSummaryMessage → ToolUseSummary', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.ToolUseSummary);

      const tus = event as GADToolUseSummaryEvent;
    ...
    it('maps SDKTaskStartedMessage → TaskStarted', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.TaskStarted);

      const ts = event as GADTaskStartedEvent;
    ...
    it('maps SDKTaskProgressMessage → TaskProgress', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.TaskProgress);

      const tp = event as GADTaskProgressEvent;
    ...
    it('maps SDKTaskNotificationMessage → TaskNotification', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.TaskNotification);

      const tn = event as GADTaskNotificationEvent;
    ...
    it('maps SDKAPIRetryMessage → APIRetry', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.APIRetry);

      const retry = event as GADAPIRetryEvent;
    ...
    it('maps SDKRateLimitEvent → RateLimit', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.RateLimit);

      const rl = event as GADRateLimitEvent;
    ...
    it('maps SDKStatusMessage → StatusChange', () => {
      ...
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.StatusChange);
      expect((event as GADStatusChangeEvent).status).toBe('compacting');
    });

    it('maps SDKCompactBoundaryMessage → CompactBoundary', () => {
      const event = stream.mapSDKMessage(makeCompactBoundary());
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.CompactBoundary);

      const cb = event as GADCompactBoundaryEvent;
  ...
  describe('transport management', () => {
    it('delivers events to subscribed transports', () => {
      ...
      expect(received[0]!.type).toBe(GADEventType.SessionInit);
    });

    it('delivers events to multiple transports', () => {
      const received1: GADEvent[] = [];
      const received2: GADEvent[] = [];
  ...
  describe('EventEmitter integration', () => {
    it('emits typed events via "event" channel', () => {
      ...
      expect(events).toHaveLength(2);
      expect(events[0]!.type).toBe(GADEventType.SessionInit);
      expect(events[1]!.type).toBe(GADEventType.SessionComplete);
    });

    it('emits events on per-type channels', () => {
      const initEvents: GADEvent[] = [];
      stream.on(GADEventType.SessionInit, (e: GADEvent) => initEvents.push(e));

      stream.mapAndEmit(makeSystemInit());
      stream.mapAndEmit(makeResultSuccess());

      expect(initEvents).toHaveLength(1);
      expect(initEvents[0]!.type).toBe(GADEventType.SessionInit);
    });
  });
  ...
  describe('stream_event mapping', () => {
    it('maps SDKPartialAssistantMessage → StreamEvent', () => {
      ...
      const event = stream.mapSDKMessage(msg);
      expect(event).not.toBeNull();
      expect(event!.type).toBe(GADEventType.StreamEvent);
      expect((event as GADStreamEvent).event).toEqual({ type: 'content_block_delta' });
    });
  });
