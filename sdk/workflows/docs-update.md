...
- gad-doc-writer — Writes and updates project documentation files
- gad-doc-verifier — Verifies factual claims in docs against the live codebase
</available_agent_types>
...
```bash
INIT=$(gad-tools docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(gad-tools agent-skills gad-doc-writer 2>/dev/null)
```
...
- These will be passed to gad-doc-verifier in the verify_docs step for accuracy checking
- If inaccuracies are found, they will be dispatched to gad-doc-writer in `fix` mode for surgical corrections
...
4. For each gap the user selects:
   ...
   - The gad-doc-writer will receive a `doc_assignment` with `type: "custom"` and a description of what to document, using the project's source files as content discovery targets
...
```
{filename} appears to be hand-written (no GAD marker found).
...
Spawn 3 parallel gad-doc-writer agents for Wave 1 docs: README, ARCHITECTURE, CONFIGURATION.
...
Spawn a `gad-doc-writer` agent with `run_in_background=true`:
...
**Fallback when Task tool is unavailable:** Generate per-package READMEs sequentially inline after the `sequential_generation` step. For each package directory with a `package.json`, construct the equivalent `doc_assignment` block and generate the README following gad-doc-writer instructions.
...
Read `agents/gad-doc-writer.md` instructions once before beginning. Follow the create_mode or update_mode instructions from that agent for each doc, using the same doc_assignment fields as the parallel path.
...
1. **README** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase (Read, Grep, Glob, Bash) following gad-doc-writer create_mode / update_mode instructions
...
2. **ARCHITECTURE** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase following gad-doc-writer instructions
...
- Follow gad-doc-writer instructions for per_package scope
...
1. Spawn the `gad-doc-verifier` agent (or invoke sequentially if Task tool is unavailable) with a `<verify_assignment>` block:
   ```xml
   <verify_assignment>
...
1. Spawn the `gad-doc-verifier` agent with the same `<verify_assignment>` block as above.
...
Non-canonical docs with failures ARE eligible for the fix_loop. When a non-canonical doc has `claims_failed > 0`, dispatch it to gad-doc-writer in `fix` mode with the failures array — the writer's fix mode does surgical corrections on specific lines regardless of doc type (no template needed). The writer MUST NOT restructure, rephrase, or reformat any content beyond the failing claims.
...
1. For each doc with `claims_failed > 0` in the latest verification_results:
   ...
   b. Spawn `gad-doc-writer` agent (or invoke sequentially) with a fix assignment:
      ```xml
      <doc_assignment>
...
Invoke the gad-doc-verifier agent in read-only mode for each file in `existing_docs` from the init JSON:

1. For each doc in `existing_docs`:
   a. Spawn `gad-doc-verifier` (or invoke sequentially if Task tool is unavailable) with:
      ```xml
      <verify_assignment>
      doc_path: {doc.path}
      project_root: {project_root from init JSON}
...
Display note:
```
To fix failures automatically: /gad:docs-update (runs generation + fix loop)
To regenerate all docs from scratch: /gad:docs-update --force
```
...
3. Re-run /gad:docs-update to regenerate clean docs
```
...
```bash
gad-tools commit "docs: generate project documentation" \
...
```
Run `/gad:docs-update --verify-only` to fact-check generated docs against the codebase.
```
...
- [ ] Generated docs contain zero GAD methodology content
...
- [ ] Hand-written docs (no GAD marker) prompted for preserve/supplement/regenerate before dispatch (unless --force)
...
- [ ] --verify-only invokes gad-doc-verifier for full fact-checking (not just VERIFY marker count)
</success_criteria>
