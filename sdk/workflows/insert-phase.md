...
Example: `/gad:insert-phase 72 Fix critical auth bug`
-> after = 72
...
Usage: /gad:insert-phase <after> <description>
Example: /gad:insert-phase 72 Fix critical auth bug
```
...
```bash
INIT=$(gad-tools init phase-op "${after_phase}")
...
**Delegate the phase insertion to gad-tools:**

```bash
RESULT=$(gad-tools phase insert "${after_phase}" "${description}")
```
...
- [ ] `gad-tools phase insert` executed successfully
