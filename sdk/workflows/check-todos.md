...
- `/gad:check-todos` → show all
- `/gad:check-todos api` → filter to area:api only
</step>
...
- `/gad:check-todos [area]` to filter by area
- `q` to exit
```
...
gad-tools commit "docs: start work on todo - [title]" --files .planning/todos/completed/[filename] .planning/STATE.md
```
