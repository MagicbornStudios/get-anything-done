...
These will carry forward as debt. Review: `/gad:audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to gad-tools:**

```bash
TRANSITION=$(gad-tools phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `gad-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(gad-tools progress bar --raw)
```
...
**Use the transition result from `gad-tools phase complete`:**

The `is_last_phase` field from the phase complete result tells you directly:
- `is_last_phase: false` → More phases remain → Go to **Route A**
- `is_last_phase: true` → Last phase done → **Route directly to milestone completion logic in shipped GAD**
...
**Workstream collision check (historical upstream branch):**

GAD does **not** currently ship workstream support. Do not call
`gad-tools workstream ...` here and do not branch on `GSD_WORKSTREAM`.

This workflow inherited an upstream workstream branch before the command surface
was removed. For the shipped GAD workflow, treat `is_last_phase: true` as a
direct route to **Route B** until workstreams are formally restored and
evaluated.
...
Exit skill and invoke SlashCommand("/gad:plan-phase [X+1] --auto ${GSD_WS}")
...
Exit skill and invoke SlashCommand("/gad:discuss-phase [X+1] --auto ${GSD_WS}")
...
`/gad:discuss-phase [X+1] ${GSD_WS}` — gather context and clarify approach
...
**Also available:**
- `/gad:plan-phase [X+1] ${GSD_WS}` — skip discussion, plan directly
- `/gad:research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
**Also available:**
- `/gad:discuss-phase [X+1] ${GSD_WS}` — revisit context
- `/gad:research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
```bash
gad-tools config-set workflow._auto_chain_active false
```
...
Exit skill and invoke SlashCommand("/gad:complete-milestone {version} ${GSD_WS}")
...
`/gad:complete-milestone {version} ${GSD_WS}`
