...
<step name="load">
**Use structured extraction from gad-tools:**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(gad-tools roadmap analyze)`
- `STATE=$(gad-tools state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  gad-tools summary-extract <path> --fields one_liner
  ```
...
<step name="report">
**Generate progress bar from gad-tools, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(gad-tools progress bar --raw)
```
...
## Pending Todos
- [count] pending — /gad:check-todos to review
...
```bash
DEBT=$(gad-tools audit-uat --raw 2>/dev/null)
```
...
Review: `/gad:audit-uat ${GSD_WS}` — full cross-phase audit
Resume testing: `/gad:verify-work {phase} ${GSD_WS}` — retest specific phase
```
...
```bash
PHASE_SECTION=$(gad-tools roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/gad:plan-phase {phase-number} ${GSD_WS}`
...
- `/gad:list-phase-assumptions {phase}` — see Claude's assumptions
...
`/gad:discuss-phase {phase} ${GSD_WS}` — gather context and clarify approach
...
**Also available:**
- `/gad:plan-phase {phase} ${GSD_WS}` — skip discussion, plan directly
- `/gad:list-phase-assumptions {phase} ${GSD_WS}` — see Claude's assumptions
...
`/gad:plan-phase {phase} --gaps ${GSD_WS}`
...
**Also available:**
- `/gad:execute-phase {phase} ${GSD_WS}` — execute phase plans
- `/gad:verify-work {phase} ${GSD_WS}` — run more UAT testing
...
`/gad:verify-work {phase} ${GSD_WS}` — resume testing from where you left off
...
**Also available:**
- `/gad:audit-uat ${GSD_WS}` — full cross-phase UAT audit
- `/gad:execute-phase {phase} ${GSD_WS}` — execute phase plans
...
```bash
NEXT_PHASE_SECTION=$(gad-tools roadmap get-phase "$((Z+1))" 2>/dev/null)
...
`/gad:discuss-phase {Z+1} ${GSD_WS}` — gather context and clarify approach
...
**Also available:**
- `/gad:plan-phase {Z+1} ${GSD_WS}` — skip discussion, plan directly
- `/gad:verify-work {Z} ${GSD_WS}` — user acceptance test before continuing
...
**Also available:**
- `/gad:verify-work ${GSD_WS}` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/gad:plan-phase [next] ${GSD_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/gad:resume-work ${GSD_WS}`
  </step>
