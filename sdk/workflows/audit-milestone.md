...
AGENT_SKILLS_CHECKER=$(gad-tools agent-skills gad-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(gad-tools resolve-model gad-integration-checker --raw)
```
...
PHASE_INFO=$(gad-tools find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  gad-tools summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(gad-tools config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/gad:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/gad:validate-phase {N}` |

Phases needing validation: run `/gad:validate-phase {N}` for each flagged phase.
...
- /gad:complete-milestone {version} — proceed anyway (accept tech debt)
