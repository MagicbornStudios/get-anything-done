...
/gad:execute-phase {phase} ${GSD_WS}                # Continue remaining waves
/gad:execute-phase {phase} --wave {next} ${GSD_WS}  # Run the next wave explicitly
```
...
| `gaps_found` | Present gap summary, offer `/gad:plan-phase {phase} --gaps ${GSD_WS}` |
...
`/gad:plan-phase {X} --gaps ${GSD_WS}`

<sub>`/clear` first → fresh context window</sub>

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/gad:verify-work {X} ${GSD_WS}` — manual testing first
```

Gap closure cycle: `/gad:plan-phase {X} --gaps ${GSD_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/gad:execute-phase {X} --gaps-only ${GSD_WS}` → verifier re-runs.
</step>
...
/gad:progress ${GSD_WS} — see updated roadmap
/gad:discuss-phase {next} ${GSD_WS} — discuss next phase before planning
/gad:plan-phase {next} ${GSD_WS} — plan next phase
/gad:execute-phase {next} ${GSD_WS} — execute next phase
```
