...
```bash
INIT=$(gad-tools state load 2>/dev/null)
```
...
|--------------------------|----------|-----|
| Starting a new project, "set up", "initialize" | `/gad:new-project` | Needs full project initialization |
| Mapping or analyzing an existing codebase | `/gad:map-codebase` | Codebase discovery |
| A bug, error, crash, failure, or something broken | `/gad:debug` | Needs systematic investigation |
| Exploring, researching, comparing, or "how does X work" | `/gad:research-phase` | Domain research before planning |
| Discussing vision, "how should X look", brainstorming | `/gad:discuss-phase` | Needs context gathering |
| A complex task: refactoring, migration, multi-file architecture, system redesign | `/gad:add-phase` | Needs a full phase with plan/build cycle |
| Planning a specific phase or "plan phase N" | `/gad:plan-phase` | Direct planning request |
| Executing a phase or "build phase N", "run phase N" | `/gad:execute-phase` | Direct execution request |
...
**Requires `.planning/` directory:** All routes except `/gad:new-project`, `/gad:map-codebase`, `/gad:help`, and `/gad:join-discord`. If the project doesn't exist and the route requires it, suggest `/gad:new-project` first.

**Ambiguity handling:** If the text could reasonably match multiple routes, ask the user via AskUserQuestion with the top 2-3 options. For example:

```
"Refactor the authentication system" could be:
1. /gad:add-phase — Full planning cycle (recommended for multi-file refactors)
...
Run the selected `/gad:*` command, passing `$ARGUMENTS` as args.
