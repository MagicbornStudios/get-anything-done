...
short-lived intermediate between `skills/candidates/` and `sdk/skills/`. That framing
...
| proto-skill (promoted) | `sdk/skills/<name>/SKILL.md` (with proto-skill provenance) | Skill is live and distributed — still carries its proto-skill origin in metadata |
...
gad evolution promote <slug> [--name X]    # move proto-skill → sdk/skills/, keep PROVENANCE.md
...
| Output location | `skills/proto-skills/` | `sdk/skills/` directly |
| Provenance | carries PROVENANCE.md forever | optional |
| Batching | yes | no |

Both end up in `sdk/skills/` eventually. The difference is the path by which they
