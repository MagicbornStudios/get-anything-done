...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
node "$HOME/.claude/bin/gad-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
...
```bash
node ~/.claude/bin/gad-tools.cjs commit-to-subrepo "feat(02-01): add user API" \
