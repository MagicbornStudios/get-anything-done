...
### Added
- `gad tasks stamp --status done|cancelled` now auto-closes linked handoffs. Scans `.planning/handoffs/open/` and `.planning/handoffs/claimed/` for files whose frontmatter `task_id` matches the stamped task id; moves each match to `closed/` with a `## Auto-closed` section and `completed_at` timestamp. Idempotent — already-closed handoffs are skipped; failure to close a handoff is non-fatal to the stamp. Implements GAD-T-63-56 (phase 63).
- `classifyRuntimeError` now recognises `EMFILE` / `ENFILE` / "too many open files" as `runtime_crash` (no cooldown). Previously these were returned as `unknown`, which could cause workers to leave a spurious quota-cooldown entry for gemini-cli when it walks `target/` or `node_modules/` trees on Windows. (phase 75)
...
- Project-detail tab shell closure (task 45-01): tab labels now sourced from `VOCAB.tabs` (first consumer of 45-04) and the Planning sub-tab state persists in the URL via a new `?sub=` search param (tasks default omits the param). Sub-tab buttons gain `data-cid="project-planning-subtab-<key>"`. Placeholder "coming soon" panes for decisions/roadmap now cite their specific follow-up tasks (45-07, 45-08).
- Per-project data adapter closure (task 45-02): `site/app/projects/[...id]/project-planning-data.ts` now returns notes alongside tasks/decisions/phases/requirements/state. New `PlanningNote` type, `loadProjectNotes(projectId)` standalone export, and a `collectNotes()` walker over `.planning/notes/` + `.planning/docs/` (`.md` / `.mdx`; title from first H1 or filename; 220-char snippet; mtime ISO; sorted newest-first). Unblocks 45-06..11 tab-component migrations.
- Planning sub-tab wiring (tasks 45-07, 45-08): `ProjectPlanningDecisionsTab` and `ProjectPlanningRoadmapTab` are now wired into `ProjectDetailTabContent.tsx`, replacing the "coming soon" placeholders. Both components were already written — this commit makes them visible. `/projects/[...id]?tab=planning&sub=decisions` shows 221 decisions newest-first with expand/collapse past 30; `?sub=roadmap` shows the phase list + Gantt timeline. Roadmap sub-tab remains first-class for VIEWING data, even though gad-229 demoted roadmap from the gauge bank (viewing ≠ gauging).
...
- **`gad generation open` / `gad eval open` / `gad play`** serve preserved **build artifacts** (HTML bundles) via **`lib/static-http-serve.cjs`** — not `site-compile` (so generation preview is not coupled to the planning “site” pipeline). **`--no-browser`** prints the preview URL for iframe/editor workflows. Still replaces `npx serve` (decision **gad-225**).
...
- **Decision gad-230** — visual identity palette direction: black-dominant with gold / maroon / red accents, gilded-shiny finish, non-typical and sleek. Role mapping onto the gauge bank: gold = direction, maroon = structural secondary, red = latent/alarm. Not prescriptive on hex values — specifics land in 45-14.
- **Decision gad-231** — adopt tweakcn (github.com/jnsahaj/tweakcn) as the theme generator. Standalone first (operator wants it working now), site-embed is nice-to-have. OpenAI-based theme generation likely needs tweaking; documented in the follow-up todo.
- **Decision gad-232** — VCS live showcase spec: three scripted client-side demos on the landing page (character/tileset conditional, component-remove mandatory, self-referential discovery mandatory). Deterministic, no LLM at runtime, no backend. Supersedes the VCS-showcase entry in the 45-13 follow-up.
- Three new todos: `tweakcn-integration`, `ai-asset-generation-pipeline` (`gad asset generate sprite|icon|background|favicon|og-image|tileset|avatar`), `vcs-live-showcase-three-demos`.
