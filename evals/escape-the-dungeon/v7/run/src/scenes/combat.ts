...
export function loadCombatScene(k: KAPLAYCtx) {
  k.scene("combat", () => {
    ...
    // Player HP
    k.add([k.rect(220, 16, { radius: 3 }), k.pos(60, playerY + 40), k.color(40, 20, 20)]);
    const playerHpBar = k.add([k.rect(220, 16, { radius: 3 }), k.pos(60, playerY + 40), k.color(80, 200, 80)]);
    ...
    // Player Mana
    k.add([k.rect(220, 16, { radius: 3 }), k.pos(60, playerY + 65), k.color(20, 20, 40)]);
    const playerManaBar = k.add([k.rect(220, 16, { radius: 3 }), k.pos(60, playerY + 65), k.color(80, 80, 200)]);
    ...
    // Crystals display
    k.add([
      k.text(`Crystals: ${state.player.manaCrystals}`, { size: 12 }),
      k.pos(60, playerY + 92),
      k.color(180, 200, 255),
    ]);
    ...
    function showMainActions() {
      ...
      actions.forEach((action, i) => {
        const btn = k.add([
          ...
          k.pos(actionX, actionY2 + i * btnGap),
        ...
        const label = k.add([
          ...
          k.pos(actionX + btnW / 2, actionY2 + i * btnGap + btnH / 2),
          k.anchor("center"),
          k.color(220, 220, 240),
        ]);
    ...
    function showSpellActions() {
      ...
      prepared.forEach((spell, i) => {
        ...
        const btn = k.add([
          ...
          k.pos(actionX - 50, actionY2 + i * btnGap),
          k.color(...color),
          k.area(),
        ]);
        ...
        const label = k.add([
          ...
          k.pos(actionX - 35, actionY2 + i * btnGap + btnH / 2),
          k.anchor("left"),
    ...
    function showBagActions() {
      ...
      usables.forEach((item, i) => {
        const btn = k.add([
          ...
          k.pos(actionX - 30, actionY2 + i * btnGap),
          k.color(40, 70, 50),
          k.area(),
        ]);
        ...
        const label = k.add([
          ...
          k.pos(actionX - 15, actionY2 + i * btnGap + btnH / 2),
          k.anchor("left"),
          k.color(200, 255, 200),
        ]);
      ...
      addBackButton(usables.length);
    }

    function addBackButton(afterCount: number) {
      const backBtn = k.add([
        k.rect(100, 30, { radius: 4 }),
        k.pos(actionX, actionY2 + afterCount * btnGap + 10),
        k.color(60, 60, 80),
        k.area(),
      ]);
      const backLabel = k.add([
        k.text("Back", { size: 14 }),
        k.pos(actionX + 50, actionY2 + afterCount * btnGap + 25),
        k.anchor("center"),
        k.color(200, 200, 220),
      ]);
      ...
      spell.effects?.forEach((eff) => {
        if (eff.kind === "damage" && eff.target === "enemy") {
          ...
          state.player.combatStats.currentHp = Math.min(
            ...
            state.player.combatStats.currentHp + eff.value
          );
          combatLog.push(`You cast ${spell.name} and heal ${eff.value} HP!`);
    ...
    function doShowSpells() { showSpellActions(); }
    function doShowBag() { showBagActions(); }
    ...
    function handleWin() {
      ...
      if (isBoss) {
        ...
        k.wait(2, () => {
          state.isInCombat = false;
          state.currentEnemy = null;
          // Remove enemy from room entities so they don't respawn
          k.go("game");
        });
      }
    }
