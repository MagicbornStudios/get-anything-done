...
```bash
CONFIG=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" state load)
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
