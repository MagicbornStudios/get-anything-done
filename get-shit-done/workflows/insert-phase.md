...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" phase insert "${after_phase}" "${description}")
```
