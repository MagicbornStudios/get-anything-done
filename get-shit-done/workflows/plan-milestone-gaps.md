...
HIGHEST=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
