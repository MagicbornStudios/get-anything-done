...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" init milestone-op)
...
```bash
DETAIL=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
UI_REVIEW_CFG=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
