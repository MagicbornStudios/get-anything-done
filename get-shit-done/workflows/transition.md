...
```bash
TRANSITION=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" config-set workflow._auto_chain_active false
```
