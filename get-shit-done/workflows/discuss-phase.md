...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" agent-skills gad-advisor 2>/dev/null)
```
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" resolve-model gad-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @~/.claude/agents/gad-advisor-researcher.md for your role and instructions.
...
```bash
MAX_PASSES=$(node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gad-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
